...
ArrayStream.prototype.pause = function pause() {
  if (this._debug) { console.log(this._id, 'ArrayStream.pause'); }
...
ArrayStream.prototype.resume = function resume() {
  if (this._debug) { console.log(this._id, 'ArrayStream.resume'); }
...
ArrayStream.prototype.destroy = function destroy() {
  if (this._debug) { console.log(this._id, 'ArrayStream.destroy'); }
...
ArrayStream.prototype.stream = function stream() {
  if (this._debug) { console.log(this._id, 'ArrayStream.stream'); }

  var that = this;
  function handleItem(item, cb) {
    if (that._destroyed) {
      if (that._debug) { console.log(that._id, 'ArrayStream.stream destroyed'); }
      cb(new Error('stream destroyed'));
      return;
    }

    if (that._paused) {
      if (that._debug) { console.log(that._id, 'ArrayStream.stream paused'); }
    ...
    try {
      if (!match(that._filter, item)) {
        if (that._debug) { console.log(that._id, 'ArrayStream.stream no match', JSON.stringify(item)); }
        cb();
        return;
      }
    } catch(err) {
      console.error(that._id, 'ArrayStream.stream match error', err, JSON.stringify(that._filter), JSON.stringify(item), JSON.stringify(that._items));
      cb(err);
      return;
    }

    if (that._debug) { console.log(that._id, 'ArrayStream.stream match', JSON.stringify(item)); }
    that.emit('data', item);
    cb();
  }

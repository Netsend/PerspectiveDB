...
*/
  ...
  keysItemA.forEach(function(keyA) {
    ...
    if (lca.hasOwnProperty(keyA)) {
      if (!isEqual(itemA[keyA], lca[keyA])) {
        diffA[keyA] = '~';
      }
    } else {
      diffA[keyA] = '+';
  ...
  keysItemB.forEach(function(keyB) {
    ...
    if (lcaB.hasOwnProperty(keyB)) {
      if (!isEqual(itemB[keyB], lcaB[keyB])) {
        diffB[keyB] = '~';
      }
    } else {
      diffB[keyB] = '+';
  ...
  Object.keys(diffB).forEach(function(delta) {
    if (diffA.hasOwnProperty(delta) && !isEqual(diffA[delta], diffB[delta])) {
      ...
      } else if (diffA[delta] === '~' && !isEqual(itemA[delta], itemB[delta])) {
        // both updated to different values
        conflicts.push(delta);
      } else if (diffA[delta] === '+' && !isEqual(itemA[delta], itemB[delta])) {
        // both created with different values
        conflicts.push(delta);
      } else if (diffB[delta] === '+' && itemA.hasOwnProperty(delta) && !isEqual(itemA[delta], itemB[delta])) {

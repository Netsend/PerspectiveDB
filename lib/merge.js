...
function _doMerge(itemX, itemY, lcaX, lcaY, opts) {
  ...
  opts._log.err('_doMerge itemX: %j, itemY: %j, lcaX: %j, lcaY: %j', itemX, itemY, lcaX, lcaY);
    ...
    async.reduce(lcas, firstLca, function(prevLca, currLca, cb2) {
      ...
      treeX.getByVersion(prevLca, function(err, lcaX) {
        ...
        treeY.getByVersion(currLca, function(err, lcaY) {
          ...
          log.err('merge recursive merge lcaX: %j, lcaY: %j, lcas: %j', lcaX, lcaY, lcas);
          merge(lcaX, lcaY, treeX, treeY, cb2);
        });
      });

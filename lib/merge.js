 ...
 *  - in all other cases create two merged items, bound by lca
 *
...
function merge(itemX, itemY, treeX, treeY, opts, cb) {
  ...
  log.debug('merge itemX: %j, itemY: %j', itemX, itemY);
  ...
  findLCAs(sX, sY, findLcaOpts, function(err, lcas) {
    ...
    async.eachSeries(lcas, function(lca, cb2) {
      ...
      try {
        ...
        var merge = _doMerge(itemX, itemY, prevLcaX, prevLcaY);
        log.debug('merge result x: %j, y: %j', merge[0], merge[1]);

...
function merge(itemX, itemY, treeX, treeY, opts, cb) {
  ...
  // make sure both items have a version
  if (!itemX.h.v) {
    error = new Error('itemX has no version');
    log.err('merge %s', error);
    process.nextTick(function() {
      cb(error, null);
    });
  ...
  if (!itemY.h.v) {
    error = new Error('itemY has no version');
    log.err('merge %s', error);
    process.nextTick(function() {
      cb(error, null);
    });
  ...
  findLCAs(sX, sY, findLcaOpts, function(err, lcas) {
    ...
    // fetch first lca from both trees
    var lastLcaX, lastLcaY;
    var firstLca = lcas.shift();

    log.debug('merge first lca: %j, other lcas: %j', firstLca, lcas);

    treeX.getByVersion(firstLca, function(err, lcaX) {
      if (err) { cb(err); return; }
      if (!lcaX) {
      ...
      treeY.getByVersion(firstLca, function(err, lcaY) {
        if (err) { cb(err); return; }
        if (!lcaY) {
          error = new Error('could not resolve lca');
        ...
        // recursive merge lcas, until one is left
        async.eachSeries(lcas, function(lca, cb2) {
          treeY.getByVersion(lca, function(err, lcaY) {
            ...
            log.debug('merge recursive resolved lcaY: %j', lcaY.h);

            merge(lastLcaX, lcaY, treeX, treeY, function(err, mergeX, mergeY) {
              if (err) { cb2(err); return; }
              lastLcaX = mergeX;
              lastLcaY = mergeY;
              cb2();
          ...
          log.debug('merge lastLcaX: %j, lastLcaY: %j', lastLcaX.h, lastLcaY.h);
          if (err) { cb(err); return; }
          try {
            var merge = _doMerge(itemX, itemY, lastLcaX, lastLcaY);

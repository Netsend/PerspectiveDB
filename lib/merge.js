...
var findSLCA = require('./find_single_lca');
...
/**
 * Do a three-way-merge of u and v with the single lowest common ancestor.
 *
 ...
 *
 * @param {Object} u  vertice u
 * @param {Object} v  vertice v
 * @param {stream.Readable} sU  readable stream that emits vertices from leaf to root
 * @param {stream.Readable} sV  readable stream that emits vertices from leaf to root
 * @param {Function} lookupU  function to lookup a version in the path of V
 ...
 */
function merge(u, v, sU, sV, lookupU, lookupV, opts, cb) {
  ...
  var transform = opts.transform || function(item) { return { v: item.h.v, pa: item.h.pa }; };
  var error;

  findSLCA(u, v, sU, sV, { transform: transform }, function(err, slca) {
    if (err) { cb(err); return; }

    if (!slca) {
      error = new Error('no single lca found');
      log.err('merge error %s', error);
      cb(error);
      return;
    }

    log.debug('merge lca: %j', slca);
        console.log('slca', slca);

    lookupU(slca, function(err, lcaU) {
      if (err) { cb(err); return; }
      lookupV(slca, function(err, lcaV) {
        if (err) { cb(err); return; }

        console.log('lookup u en v', lcaU, lcaV, slca);
        var merged;
        try {
          log.debug('_doMerge u: %j, v: %j, lcaU: %j, lcaV: %j', u, v, lcaU, lcaV);
          merged = _doMerge(u, v, lcaU, lcaV);

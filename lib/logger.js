...
function logger(opts, cb) {
  ...
  function log() {
    ...
    var dateMsg = date.toString() + ' ' + ident + '[' + process.pid + '] ' + prio + ': ' + msg + '\n';

    if (logFile) {
      logFile.write(dateMsg);
    }

    if (errFile && (prio <= ERR_THRESHOLD)) {
      errFile.write(dateMsg);
    }

    if (cons) {
      if (prio <= ERR_THRESHOLD) {
        process.stderr.write(dateMsg);
      } else {
        process.stdout.write(dateMsg);
      }
    }
  }

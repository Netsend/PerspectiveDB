...
function connHandler(conn) {
  ...
  log.info('client connected %s %j', connId, conn.address());

  var preauthSock = net.createConnection({ port: proxyPort, host: '127.0.0.1' }, function() {
    conn.on('text', function(data) {
      log.info('received text data "%s" %s', data, connId);
      preauthSock.write(data);
    });
...
process.once('message', function(msg) {
  ...
  logger(msg.log, function(err, l) {
    ...
    function openServerAndProceed() {
      ...
      wsServer.listen(port, host, function(err) {
        if (err) {
          log.err(err, host, port);
          log.crit('startup error %s', err);
          process.exit(1);
        }
        process.send('listen');

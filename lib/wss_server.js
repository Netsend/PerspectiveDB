...
function connHandler(conn) {
  ...
  conn.once('text', function(authReq) {
    ...
    conn.once('text', function(dataReq) {
      log.info('%s: received %d bytes', connId, dataReq.length);
  ...
  var preauthSock = net.createConnection({ port: proxyPort, host: '127.0.0.1' }, function() {
    ...
    conn.on('binary', function(rs) {
      log.info('%s: setup binary reader', connId);
      ...
      rs.on('end', function() {
        log.info('%s: end binary reader', connId);
      });
      binRecMode = true;
    });
    ...
    var ls = new LDJSONStream({ maxBytes: 512 }); // accept one data req

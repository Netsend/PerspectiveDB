...
function connHandler(conn) {
  ...
  conn.once('text', function(authReq) {
    log.info('%s: received %d bytes "%j"', connId, authReq.length, debugReq(JSON.parse(authReq)));
    received.push(authReq);
    if (connected) {
      log.info('%s: proxy auth request to server "%j"', connId, debugReq(JSON.parse(authReq)));

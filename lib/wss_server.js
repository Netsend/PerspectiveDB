...
function connHandler(conn) {
  var connId = getConnectionId(conn.socket);
  log.info('%s: connected %j', connId, conn.socket.address());

  var connected, received = [];
  conn.once('text', function(authReq) {
    log.debug2('%s: received %d bytes', connId, authReq.length);
    received.push(authReq);
    if (connected) {
      log.debug2('%s: proxy auth request to server', connId);
      preauthSock.write(received.shift());
    }
    conn.once('text', function(dataReq) {
      log.debug2('%s: received %d bytes "%s"', connId, dataReq.length, dataReq);
      received.push(dataReq);
      if (connected) {
        log.debug2('%s: proxy auth/data request(s) to server', connId);
  ...
  var preauthSock = net.createConnection({ port: proxyPort, host: '127.0.0.1' }, function() {
    log.debug2('%s: connected to proxy', connId);
    connected = true;
    if (received.length) {
      log.debug2('%s: proxy auth/data requests to server', connId);
      while (received.length) {
        preauthSock.write(received.shift());
      }
    }
    ...
    conn.on('binary', function(rs) {
      log.debug2('%s: setup binary reader', connId);
      rs.on('readable', function() {
        var chunk;
        while (null !== (chunk = rs.read())) {
          log.debug2('%s: write chunk from client', connId);
          preauthSock.write(chunk);
        }
      });
      rs.on('end', function() {
        log.debug2('%s: end binary reader', connId);
      });
      binRecMode = true;
    });
    ...
    preauthSock.pipe(ls).once('readable', function() {
      ...
      log.debug2('%s: proxy data request to client "%s"', connId, dataReq);
      ...
      if (ls.buffer.length) {
        log.debug2('push back %d bytes', ls.buffer.length);
    ...
    preauthSock.on('end', function() {
      log.debug2('%s: preauth end', connId);
      preauthClose = true;
      if (!connClose) {
        conn.close();
        connClose = true;
...
process.once('message', function(msg) {
  ...
  logger(msg.log, function(err, l) {
    ...
    try {
      ...
      log.debug2('wss changed root to %s and user:group to %s:%s', newRoot, user, group);
    } catch(err) {
    ...
    function openServerAndProceed() {
      ...
      wsServer.socket.on('tlsClientError', function(err) {
        log.notice('tlsClientError', err);
      });

      wsServer.socket.on('newSession', function() {
        log.debug('newSession');
      });

      wsServer.socket.on('resumeSession', function() {
        log.debug('resumeSession');
      });

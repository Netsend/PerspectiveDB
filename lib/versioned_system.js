...
VersionedSystem.prototype.sendPR = function sendPR(ns, pullReq, cb) {
  ...
  Replicator.fetchFromDb(replicationColl, 'import', pullReq.database, function(err, replCfg) {
    ...
    that._log.info('vs sendPR pull request forwarded', pullReq);
...
VersionedSystem.prototype.listen = function listen(user, newRoot, preauthCfg, cb) {
  ...
  function handleMessage(req, conn) {
    that._log.info('vs listen', conn.bytesRead, conn.bytesWritten, conn.address());
    ...
    that._verifyAuthRequest(req, function(err, replCfg) {
      ...
      that._log.info('vs listen push request forwarded', pushReq);
  ...
  // open preauth server
  var preauth = fork(process.execPath, [__dirname + '/preauth_exec'], {
    stdio: [0, this._log.getFileStream() || 1, this._log.getErrorStream() || 2, 'ipc'],
    env: {}
  });
  this._log.info('vs listen preauth forked');
 ...
 *   [hookPaths]:     {Array, default [hooks, local/hooks]} // list of paths to load hooks from
...
VersionedSystem.prototype._startVC = function _startVC(config, cb) {
  ...
  var vce = fork(process.execPath, [__dirname + '/versioned_collection_exec'], {
    stdio: ['pipe', this._log.getFileStream() || 'pipe', this._log.getErrorStream() || 'pipe', 'ipc'],
    env: {}
  });
...
VersionedSystem.prototype._verifyAuthRequest = function _verifyAuthRequest(req, cb) {
  ...
  User.find(usersColl, req.username, req.database, function(err, user) {
    ...
    that._log.info('vs _verifyAuthRequest user found', ns, user);

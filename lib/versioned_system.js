...
VersionedSystem.prototype.info = function info(extended, cb) {
  ...
  async.each(Object.keys(this._vcs), function(key, cb2) {
    that._vcs[key]._collection.stats(function(err, resultCollection) {
      if (err) {
      ...
      that._vcs[key]._snapshotCollection.stats(function(err, resultSnapshotCollection) {
        if (err) {
          if (err.message !== 'ns not found' && !/^Collection .* not found/.test(err.message)) {
            cb2(err);
            return;
        ...
        if (extended) {
          that._vcs[key]._snapshotCollection.count({ '_m3._ack': true }, function(err, count) {
    ...
    async.eachSeries(Object.keys(vcs[dbName]), function(collectionName, cb3) {
      ...
      vc.once('message', function(msg) {
        ...
        vc.once('message', function(msg) {
          ...
          that._ensureSnapshotAndOplogOffset(that._oplogDb.db(dbName), vceCfg, function(err, offset) {
            ...
            var or = new OplogReader(that._oplogCollection, vc.ns, opts);
            or.pipe(vc.stdin);
 ...
 *
 * @param {String} ns  namespace to work on
 * @param {String} ns  namespace to work on
 * @param {Function} cb  Called when oplog is connected to the vc. First parameter
 *                       will be an error object or null.
 */
VersionedSystem.prototype._ensureSnapshotAndOplogOffset = function _ensureSnapshotAndOplogOffset(db, vcCfg, cb) {
  ...
  this._oplogDb.findOne({}, { sort: { $natural: 1 } }, function(err, oldestOplogItem) {
    if (err) { cb(err); return; }

    var oplogOffset = oldestOplogItem;

    // get newest item from oplog
    that._oplogDb.findOne({}, { sort: { $natural: -1 } }, function(err, newestOplogItem) {
      if (err) { cb(err); return; }
      ...
      vc._snapshotCollection.count(function(err, items) {
        ...
        if (!items) { oplogOffset = newestOplogItem.ts; }

        // get max oplog pointer from snapshot
        vc.maxOplogPointer(function(err, snapshotOffset) {
          if (err) { cb(err); return; }

          snapshotOffset = snapshotOffset || new Timestamp(0, 0);

          if (snapshotOffset.greaterThan(oplogOffset)) {
            oplogOffset = snapshotOffset;
          }

          vc.ensureSnapshotCollection(vcCfg.size, function(err) {
            if (err) { cb(err); return; }

...
VersionedSystem.prototype.sendPR = function sendPR(ns, pullReq, cb) {
  ...
  Replicator.fetchFromDb(replicationColl, 'import', pullReq.database, function(err, replCfg) {
    ...
    if (!pullRequest.valid(pullReq)) {
      that._log.err('vs sendPR unable to construct a valid pull request', debugReq(pullReq));
      cb(new Error('unable to construct a valid pull request'));
      return;
    }

    that._log.info('vs sendPR pull request forwarded', debugReq(pullReq));
...
VersionedSystem.prototype.listen = function listen(user, newRoot, preauthCfg, cb) {
  ...
  function connErrorHandler(conn, e) {
    try {
      ...
      that._log.err('vs listen', JSON.stringify(conn.address()), error, e);
  ...
  function handleMessage(req, conn) {
    that._log.info('vs listen', conn.bytesRead, conn.bytesWritten, JSON.stringify(conn.address()));

    if (!authRequest.valid(req)) {
      that._log.err('vs listen invalid auth request', debugReq(req));
    ...
    that._verifyAuthRequest(req, function(err, replCfg) {
      if (err) {
        that._log.err('vs listen auth request err', debugReq(req), err);
      ...
      if (!pushRequest.valid(pushReq)) {
        that._log.err('vs listen unable to construct a valid push request', pushReq, debugReq(req));
      ...
      that._log.info('vs listen push request forwarded', debugReq(pushReq));
 ...
 *                                  // converted to B
 *   log:             {Object}      // log configuration object, child.stdout
...
VersionedSystem.prototype._startVC = function _startVC(config, cb) {
  ...
  if (typeof config.log !== 'object') { throw new TypeError('config.log must be an object'); }
  ...
  // send initial log config after vce is ready to receive messages
  vce.once('message', function(msg) {
    if (msg !== 'log1') {
      that._log.err('vs _startVC expected message "log1"', ns, msg);
      cb(new Error('expected first message to be "log1"'));
      return;
    }

    phase = 'log1';

    vce.send(config.log, config.log.file);

    // send optional error log file descriptor
    vce.once('message', function(msg) {
      if (msg !== 'log2') {
        that._log.err('vs _startVC expected message "log2"', ns, msg);
        cb(new Error('expected second message to be "log2"'));
        return;
      }

      phase = 'log2';

      vce.send('errFile', config.log.error);

      // send db and vc config after vce is ready to receive messages
      vce.once('message', function(msg) {
        if (msg !== 'init') {
          that._log.err('vs _startVC expected message "init"', ns, msg);
          cb(new Error('expected third message to be "init"'));
          return;
        }
        ...
        vce.once('message', function(msg) {
          if (msg !== 'listen') {
            ...
            cb(new Error('expected fourth message to be "listen"'));
            return;
          }
...
VersionedSystem.prototype._verifyAuthRequest = function _verifyAuthRequest(req, cb) {
  ...
  User.find(usersColl, req.username, req.database, function(err, user) {
    ...
    that._log.info('vs _verifyAuthRequest user found', ns, JSON.stringify(user));

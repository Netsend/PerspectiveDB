...
VersionedSystem.prototype.sendPR = function sendPR(ns, pullReq, cb) {
  ...
  if (typeof this._vces[ns] !== 'object') { console.error(ns); throw new Error('no versioned collection found for this combination of database and collection'); }
  ...
  Replicator.fetchFromDb(replicationColl, 'import', pullReq.database, function(err, replCfg) {
    ...
    if (!replCfg.collections[localColl]) {
      var error = 'requested collection has no import replication config';

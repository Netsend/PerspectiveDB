...
VersionedSystem.prototype._startVC = function _startVC(config, cb) {
  ...
  vce.once('message', function(msg) {
    ...
    phase = 'log1';
    that._log.info('vs _startVC "log1" received', ns);

    vce.send(config.logCfg, config.logCfg.file);
    ...
    vce.once('message', function(msg) {
      ...
      phase = 'log2';
      that._log.info('vs _startVC "log2" received', ns);

      vce.send('errFile', config.logCfg.error);

...
VersionedSystem.prototype.createServer = function createServer(user, newRoot, preauthCfg, cb) {
  ...
  function connErrorHandler(conn, err) {
    console.error('vs createServer connection error', conn.address(), err);
  ...
  function handleMessage(req, conn) {
    if (that.debug) { console.log('vs createServer', conn.bytesRead, conn.bytesWritten, conn.address()); }
    ...
    that._verifyAuthRequest(req, function(err, replCfg) {
      ...
      if (!pushRequest.valid(pushReq)) {
        if (!that._hide) { console.error('vs createServer unable to construct a valid push request', pushReq, debugReq(req)); }

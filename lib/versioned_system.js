...
VersionedSystem.prototype.sendPR = function sendPR(ns, pullReq, cb) {
  ...
  if (typeof this._vces[ns] !== 'object') {
    ...
    if (!that._hide) { console.error('vs sendPR', ns, error); }
  ...
  Replicator.fetchFromDb(replicationColl, 'import', pullReq.database, function(err, replCfg) {
    ...
    if (!pullRequest.valid(pullReq)) {
      if (!that._hide) { console.error('vs sendPR unable to construct a valid pull request', debugReq(pullReq)); }
      cb(new Error('unable to construct a valid pull request'));
      return;
    }

    if (that.debug) { console.log('vs sendPR pull request forwarded', pullReq); }
...
VersionedSystem.prototype.listen = function listen(user, newRoot, preauthCfg, cb) {
  ...
  function connErrorHandler(conn, e) {
    try {
      ...
      console.error('vs listen', JSON.stringify(conn.address()), error, e);
  ...
  function handleMessage(req, conn) {
    ...
    if (!authRequest.valid(req)) {
      if (!that._hide) { console.error('vs listen invalid auth request', debugReq(req)); }
    ...
    that._verifyAuthRequest(req, function(err, replCfg) {
      if (err) {
        if (!that._hide) { console.error('vs listen auth request err', debugReq(req), err); }
      ...
      if (that.debug) { console.log('vs listen push request forwarded', pushReq); }
  ...
  if (this.debug) { console.log('vs listen preauth forked'); }
...
VersionedSystem.prototype._verifyAuthRequest = function _verifyAuthRequest(req, cb) {
  ...
  User.find(usersColl, req.username, req.database, function(err, user) {
    ...
    if (!user) {
      error = 'invalid credentials';
      if (!that._hide) { console.error('vs _verifyAuthRequest user not found', ns, error); }
      cb(new Error(error));
      return;
    }

    if (that.debug) { console.log('vs _verifyAuthRequest %s user found', ns, user); }

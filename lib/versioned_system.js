  ...
  async.eachSeries(Object.keys(vcs), function(dbName, cb2) {
    async.eachSeries(Object.keys(vcs[dbName]), function(collectionName, cb3) {
      ...
      if (that._oplogReaders[ns]) {
        error = new Error('vc is already initialized');
      ...
      vc.stdout.on('data', function(data) {
        if (!that._hide) { console.log('vs vc stdout', ns, data); }
      });
      vc.stderr.on('data', function(data) {
        if (!that._hide) { console.error('vs vc stderr', ns, data); }
      });
...
VersionedSystem.prototype.stop = function stop(cb) {
  ...
  if (this._preauth) {
    tasks.push(function(cb2) {
      that._preauth.on('exit', function(code) {
        if (code !== 0) {
          cb2(new Error('preauth exited with non-zero status'));
          return;
        }
        cb2();
      });
  ...
  ors.forEach(function(key) {
    ...
    tasks.push(function(cb2) {
      console.log('closing oplog reader');
      or.on('end', cb2);
      or.close();
    });
  });

  var vcs = Object.keys(this._vcs);
  vcs.forEach(function(key) {
    var vc = that._vcs[key];
    tasks.push(function(cb2) {
      vc.on('exit', function(code) {
        if (code !== 0) {
          cb2(new Error('vc exited with non-zero status'));
          return;
        }
        cb2();
      });
      console.log('killing vc');
      vc.kill();
    });
  });

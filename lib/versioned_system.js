 ...
 *     [file]          {String|Number|Object}  log all messages to this file,
 *                                  // either a filename, file descriptor or
 *                                  // writable stream.
...
VersionedSystem.prototype.listen = function listen(user, newRoot, preauthCfg, cb) {
  ...
  if (typeof preauthCfg.logCfg !== 'object') { throw new TypeError('preauthCfg.logCfg must be an object'); }
  if (typeof cb !== 'function') { throw new TypeError('cb must be a function'); }
  ...
  that._log.info('vs listen preauthCfg %j', preauthCfg);

  that._log.info('vs listen forking preauth');

  var stdio = ['pipe', 'pipe', 'pipe', 'ipc'];

  // use fd 4 if a file stream is opened
  if (preauthCfg.logCfg.file) {
    stdio[4] = preauthCfg.logCfg.file.fd;
    preauthCfg.logCfg.file = 4;
  }

  // use fd 5 if an error stream is opened
  if (preauthCfg.logCfg.error) {
    stdio[5] = preauthCfg.logCfg.error.fd;
    preauthCfg.logCfg.error = 5;
  }
  ...
  preauth.once('message', function(msg) {
    ...
    that._log.info('vs listen "init" received');
    ...
    preauth.once('message', function(msg) {
      ...
      that._log.info('vs listen "listen" received');

...
function Tree(db, name, opts) {
  ...
  this._idxHeadPrefix = new Buffer('idx/head\x00' + name);
...
Tree.prototype._validParents = function _validParents(item, cb) {
  ...
  async.every(item._h.pa, function(pa, cb2) {
    ...
    that._log.info('vc _validParents dbData descending DAG %s search for %s', item._h.id, item._h.v);

    var r = Tree.getRange(that._dbDataPrefix, Tree._idToB(item._h.id));
    ...
    it.on('data', function(sItem) {
      ...
      that._log.info('vc _validParents dbData descend item %j', sItem._h);

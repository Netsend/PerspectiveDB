...
Tree.prototype.iterateInsertionOrder = function iterateInsertionOrder(opts, iterator, cb) {
  ...
  if (opts.v) {
    ...
    tasks.push(function(cb2) {
      ...
      that._db.get(vKey, function(err, dsKey) {
        ...
        var key = Tree.parseKey(dsKey);
        that._log.debug('tree iterateInsertionOrder resolved opts.v to i %s', key.i);
        offset = key.i;
        cb2();
      });
    });
  ...
  async.series(tasks, function(err) {
    ...
    var it = that._db.createReadStream({ gte: r.s, lte: r.e });
    ...
    it.on('data', function(obj) {
      ...
      that._db.get(dsKey, function(err, item) {
        ...
        iterator(opts.raw ? item : BSON.deserialize(item), function(cont) {

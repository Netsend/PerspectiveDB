 ...
 *
 * @param {Buffer} [id]  optional id to prefix start and end
 * @param {Function} iterator  function called with each head
 * @param {Function} cb  First parameter will be an error object or null.
 */
Tree.prototype.getHeads = function getHeads(id, iterator, cb) {
  ...
  if (id && !Buffer.isBuffer(id)) { throw new TypeError('id must be a buffer'); }
  ...
  it.on('data', function(headKey) {
    it.pause();
    var key = Tree.parseKey(headKey.key, null, 'base64');
    var val = Tree.parseKey(headKey.value);

    that._log.debug('tree getHeads iterate key %j', key);
 ...
 *
 * opts:
 *   keepOpen {Boolean, default false}  optional offset, valid lbeint
...
Tree.prototype.createReadStream = function createReadStream(opts) {
  opts = opts || {};
  if (typeof opts !== 'object' || opts === null || Array.isArray(opts)) { throw new TypeError('opts must be an object'); }
  if (typeof opts.keepOpen !== 'undefined' && typeof opts.keepOpen !== 'boolean') { throw new TypeError('opts.keepOpen must be a boolean'); }
  ...
  // ensure key range is always async
  ...
  async.series(tasks, function(err) {
    ...
    it.on('data', function(obj) {
      ...
      that._db.get(dsKey, function(err, item) {
        ...
        that._log.debug('tree createReadStream ds item push %j', item);

        if (readable.push(item)) {
          it.resume();
          that._log.debug('tree createReadStream resume');
        } else {
          that._log.debug('tree createReadStream wait for drain');
    ...
    // when the end of the ikey index is reached, close or pipe all data events directly to the stream
    it.on('close', function() {
      that._log.debug('tree createReadStream ikey range close');
      if (opts.keepOpen) {
        that._log.debug('tree createReadStream keep open');
        that.pipe(readable);
      } else {
        that._log.debug('tree createReadStream calling readable.end()');
        readable.end();
      }
    });
  });
  ...
  readable.on('finish', function() {
    that._log.debug('tree createReadStream writable part finished, emitting close');
    this.pause();
    this.emit('close');
  });

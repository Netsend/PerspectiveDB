...
Tree.prototype.getHeads = function getHeads(opts, iterator, cb) {
  ...
  it.on('data', function(headIdx) {
    ...
    that._log.debug('tree getHeads iterate key %s val %j', key.name.toString('utf8'), val);
...
Tree.prototype.iterateInsertionOrder = function iterateInsertionOrder(opts, iterator, cb) {
  ...
  s.on('data', function(item) {
    ...
    iterator(item, function(err, cont) {
      if (err) { cb(err); return; }
      if (cont != null && !cont) { s.destroy(); cb(); return; }
      s.resume();
    });
  });
...
Tree.prototype.createReadStream = function createReadStream(opts) {
  ...
  function scheduleNextOnEnd() {
    that._log.debug('tree createReadStream schedule next');
    running = false;
    rs.unpipe(ws);
  ...
  function scheduleEndOnEnd() {
    that._log.debug('tree createReadStream schedule end');
    running = false;
    rs.unpipe(ws);
    ws.end();
  }
  ...
  function setupNewReaderToWriter() {
    that._log.debug('tree createReadStream open reader');
  ...
  if (opts.tail) {
    ...
    ws.close = function() {
      that._log.debug('tree createReadStream close');
      if (running) {
        rs.removeListener('end', scheduleNextOnEnd);
        rs.once('end', scheduleEndOnEnd);
      } else {
        that._log.debug('tree createReadStream end now');
...
Tree.prototype._resolveVtoI = function _resolveVtoI(version, cb) {
  ...
  this._getDsKeyByVersion(version, function(err, dsKey) {
    ...
    if (!dsKey) {
      ...
      that._log.err('tree _resolveVtoI %s: %s', error, version);
      cb(error);
      return;
    }

    var i = Tree.parseKey(dsKey).i;
    that._log.debug('tree _resolveVtoI %s => %s', version, i);
    cb(null, i);
  });
};
  ...
  async.filterSeries(items, function(chunk, cb2) {
    ...
    that._validNewItem(item, function(err, valid, missingParents, exists) {
      ...
      if (!valid && missingParents.length) {
        if (missingParents.every(function(pa) {
          ...
          that._log.info('tree _writev parent(s) are not in DAG but in batch, set valid %j', item.h);
          valid = true;
        } else {
          that._log.info('tree _writev parent(s) are not in DAG and not in batch %j', item.h);
        }
      }
    ...
    async.eachSeries(fitems, function(chunk, cb2) {
      ...
      that._nextI(function(err, i) {
        ...
        that._log.info('tree _writev "%s" new data item %j', that.name, item);
        ...
        that._log.debug('tree _writev iKey: %s, vKey: %s, headKey: %s, dsKey: %s', iKey.toString('hex'), vKey.toString('hex'), headKey.toString('hex'), dsKey.toString('hex'));
...
Tree.prototype._validNewItem = function _validNewItem(item, cb) {
  ...
  this._log.debug('tree _validNewItem id: %s, version: %s, parents: %s', id, version, parents);
  ...
  this._db.get(vKey, function(err, dsKey) {
    if (err && !err.notFound) {
      that._log.err('tree _validNewItem item lookup error %j', err);
      cb(err);
      return;
    }
    ...
    if (!err) {
      ...
      if (key.id !== id) {
        ...
        that._log.err('tree _validNewItem %j %s %s', error, key.id, version);
        cb(error);
        return;
      } else {
        // the version does already exist
        that._log.debug('tree _validNewItem version already exists %s', version);
        cb(null, false, [], true);
        return;
      }
    }

    that._log.debug('tree _validNewItem good, this version does not exist in the tree');
    ...
    if (parents.length) {
      ...
      async.each(parents, function(pa, cb2) {
        that._log.info('tree _validNewItem parent lookup %s', pa);

        // check if this parent exists
        that._db.get(that._composeVKey(pa), function(err, dsKey) {
          if (err && !err.notFound) {
            that._log.err('tree _validNewItem parent lookup error %j', err);
            cb2(err);
            return;
          }

          if (err && err.notFound) {
            that._log.debug('tree _validNewItem missing parent %s', pa);
          ...
          if (key.id !== id) {
            ...
            that._log.err('tree _validNewItem %j %s %s', error, key.id, pa);
            cb2(error);
            return;
          }
      ...
      it.on('data', function(headVal) {
        ...
        if (parsedHeadVal.d) {
          that._log.debug('tree _validNewItem missingParents head deleted %j continue...', parsedHeadVal);
        } else {
          that._log.info('tree _validNewItem non-deleted head exists %j for %j', parsedHeadVal, item.h);
          // not deleted

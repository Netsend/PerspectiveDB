 ...
 */
Tree.prototype.getIKeyRange = function getIKeyRange() {
      ...
      that._nextI(function(err, i) {
        ...
        that._db.batch(tra, function(err) {
          ...
          if (err) {
            ...
            that._log.info('tree _write item written %s %j', i, item._h);
            that.emit('data', item);
          }
          cb(err);
        });
...
Tree._idToB = function _idToB(id) {
  ...
  return new Buffer(Tree._ensureString(id));
};
...
Tree.prototype._decomposeIKey = function _decomposeIKey(b) {
  ...
  if (b[this._idxIPrefix.length - 1] !== IKEY) {
    this._log.err('tree _decomposeIKey %j', b);
    throw new TypeError('b key must be of key subtype 0x02');
  }
...
Tree.prototype._isNewChild = function _isNewChild(item, cb) {
  ...
  this._db.get(vKey, function(err, dsKey) {
    if (err && !err.notFound) {
      that._log.err('tree _isNewChild item lookup error %j %j', item._h, err);
      that.emit('error', err);
      cb(err);
      return;
    }
    ...
    if (!err) {
      ...
      if (key.id !== item._h.id) {
        ...
        that._log.err('tree _isNewChild %j %s %j', error, key.id, item._h);
        that.emit('error', error);
        cb(error);
        return;
      } else {
        // the version does already exist
        that._log.debug('tree _isNewChild version already exists %j', item._h);
        cb(null, false);
        return;
      }
    }
    ...
    async.every(item._h.pa, function(pa, cb2) {
      ...
      that._log.info('tree _isNewChild ds descending DAG %s search for %s', item._h.id, item._h.v);

      var r = that.getDsKeyRange(Tree._idToB(item._h.id));
      ...
      it.on('data', function(sItem) {
        ...
        that._log.debug('tree _isNewChild ds descend item %j', sItem._h);

        if (sItem._h.v === pa) {
          that._log.info('tree _isNewChild version equals needle %s', pa);
          // great success
          parentFound = true;
          it.destroy();
        } else {
          that._log.debug('tree _isNewChild version %s does not equal needle %s', sItem._h.v, pa);
        }
      });
      ...
      var r = that.getHeadKeyRange(Tree._idToB(item._h.id));
      var it = that._db.createValueStream({ reverse: true, gte: r.s, lt: r.e });
...
Tree.prototype._validParents = function _validParents(item, cb) {
  ...
  async.every(item._h.pa, function(pa, cb2) {
    ...
    it.on('data', function(sItem) {
      sItem = BSON.deserialize(sItem);
      that._log.debug('tree _validParents ds descend item %j', sItem._h);
    ...
    that._log.debug('tree _validParents all parents exist');
    ...
    that._log.debug('tree _validParents since this is a new root, search for exising items');

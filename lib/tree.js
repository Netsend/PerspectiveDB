...
Tree.getPrefix = function getPrefix(name, type) {
  ...
  var prefix = new Buffer(1 + namelen + 1 + 1);

  prefix[0] = namelen;
  prefix.write(name, 1);
  prefix[namelen + 1] = 0x00;
  prefix[namelen + 2] = type;
...
Tree.prototype.createHeadReadStream = function createHeadReadStream(opts) {
  ...
  return this._db.createReadStream({ gte: r.s, lt: r.e }).pipe(transformer);
};
...
Tree.prototype.lastByPerspective = function lastByPerspective(pe, decodeV, cb) {
  ...
  this._db.get(usKey, function(err, vkey) {
    if (err) {
      if (err.notFound) {
        that._log.debug2('t:%s lastByPerspective not found %s %j', that.name, pe, usKey);
        cb(null, null);
        return;
      } else {
    ...
    that._log.debug2('t:%s lastByPerspective %s %s', that.name, pe, v);
    cb(null, v);
  });
};
...
Tree.prototype.stats = function stats(cb) {
  ...
  this.createHeadReadStream().on('readable', function() {
    var head = this.read();

    if (!head) { // end of stream
      cb(null, {
        heads: {
          count: headCount,
          conflict: conflict,
...
/**
 * Check if a certain version is in the buffer to be written. This function uses
 * the internal Node API and therefore may break.
 *
 * @param {String|Object} id  id that is a string or implements "toString"
 * @param {Function} cb  First parameter will be an error object or null.
 */
Tree.prototype.inBufferById = function inBufferById(id) {
  var found = false;

  if (this._buffer && this._buffer.length)
    found = this._buffer.some(function(item) {
      // support "atomic" array elements (see _writev)
      if (Array.isArray(item.chunk)) { // descend
        return item.chunk.some(item2 => id === item2.h.id);
      } else {
        return id === item.chunk.h.id;
      }
    });
  ...
  // synchronous walk over the internal node writable stream buffer
  return this._writableState.getBuffer().some(function(item) {
    // support "atomic" array elements (see _writev)
    if (Array.isArray(item.chunk)) { // descend
      return item.chunk.some(item2 => id === item2.h.id);
    } else {
 ...
 * @param {Function} cb  First parameter will be an error object or null.
 */
Tree.prototype.inBufferByVersion = function inBufferByVersion(version) {
  var found = false;
  ...
  if (this._buffer && this._buffer.length)
    found = this._buffer.some(function(item) {
      // support "atomic" array elements (see _writev)
      if (Array.isArray(item.chunk)) { // descend
        return item.chunk.some(item2 => version === item2.h.v);
      } else {
        return version === item.chunk.h.v;
      }
    });
...
Tree.prototype._resolveVtoI = function _resolveVtoI(version, cb) {
  ...
  this._getDsKeyByVersion(version, function(err, dsKey) {
    ...
    if (!dsKey) {
      error = new Error('version not found');
      that._log.debug2('t:%s _resolveVtoI %s: %s', that.name, error, version);
      cb(error);
      return;
    }

    var i = Tree.parseKey(dsKey).i;
    that._log.debug2('t:%s _resolveVtoI %s => %s', that.name, version, i);
    cb(null, i);
  });
};
 ...
 * lastByPerspective.
 *
 * If chunk is an array it will be treated atomically, futhermore adjacent non-
 * array items will be grouped together in one atomic operation.
 *
...
Tree.prototype._writev = function(items, cb) {
  ...
  // save buffer for inspection by inBuffer
  this._buffer = items;
  ...
  var group = [];
  function flushGroup(cb3) {
    if (group.length) {
      that._saveAtomic(group, cb3);
      group = [];
    } else {
  ...
  // split arrays and group adjacent non-array chunks.
  async.eachSeries(items, function(chunk, cb3) {
    var item = chunk.chunk;
    if (Array.isArray(item)) {
      // first any previous items
      flushGroup(function(err) {
        if (err) { cb3(err); return; }
        that._saveAtomic(item, cb3);
      });
    } else {
      group.push(item);
      cb3();
    ...
    flushGroup(cb2); // flush any remaining options
  });
};

Tree.prototype._saveAtomic = function(items, cb) {
  ...
  // check each item
  async.filterSeries(items, function(item, cb2) {
    ...
    async.eachSeries(fitems, function(item, cb2) {
      that._nextI(function(err, i) {
        if (err) { cb2(err); return; }

        item.h.i = i;

        that._log.debug('t:%s _writev %j (%d)', that.name, item.h, fitems.length);
...
Tree.prototype._composeVKey = function _composeVKey(v) {
  ...
  if (typeof v === 'string' && Buffer.byteLength(v, 'base64') !== this._vSize) {
    var error = new Error('v must be the same size as the configured vSize');
    this._log.err('t:%s _composeVKey %s %d %s', this.name, v, this._vSize, error);
    throw error;
  }
...
Tree.prototype._composeHeadKey = function _composeHeadKey(id, v) {
  ...
  var b = new Buffer(this._idxHeadPrefix.length + Buffer.byteLength(id) + 1 + 1 + this._vSize);
  var offset = 0;
...
Tree.prototype._composeDsKey = function _composeDsKey(id, i) {
  ...
  var b = new Buffer(this._dsPrefix.length + id.length + 1 + 1 + this._iSize);
  var offset = 0;
...
Tree.prototype._validNewItem = function _validNewItem(item, batch, cb) {
  ...
  this._log.debug2('t:%s _validNewItem id: %s, version: %s, parents: %s', that.name, id, version, parents);
  ...
  if (parents.length) {
    ...
    this._db.get(vKey, function(err, dsKey) {
      ...
      that._log.debug2('t:%s _validNewItem good, this version does not exist in the tree', that.name);
    ...
    it.on('readable', function() {
      ...
      that._db.get(vKey, function(err, dsKey) {
        ...
        if (heads.length) {
          that._log.debug2('t:%s _validNewItem invalid root item %j, head(s) already exist in the tree %s', that.name, item.h, heads);
        }

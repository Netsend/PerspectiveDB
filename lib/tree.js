...
Tree.prototype.getHeadVersions = function getHeadVersions(id, cb) {
  ...
  it.on('data', function(headKey) {
    var key = Tree.parseKey(headKey, null, 'base64');
...
Tree.prototype.getHeads = function getHeads(opts, iterator, cb) {
  ...
  it.on('data', function(headIdx) {
    it.pause();
    var key = Tree.parseKey(headIdx.key, null, 'base64');
...
Tree.prototype._transform = function(item, encoding, cb) {
  ...
  this._isNewChild(item, function(err, isNew) {
    ...
    if (!isNew) {
      ...
      that._log.err('tree _transform %s %j', error, item.h);
      cb(error);
      return;
    }
    ...
    that._nextI(function(err, i) {
      ...
      item.h.pa.forEach(function(pa) {
        that._log.debug('tree _transform delete old head', item.h.id, pa);
      ...
      that._db.batch(tra, function(err) {
        ...
        if (err) {
          that._log.err('tree _transform item not written %s %j %j', i, item.h, err);
          cb(err);
          return;
        }

        that._log.info('tree _transform item written %s %j', i, item.h);
        that.push(bitem);
        cb();
      });
    });
...
Tree.prototype._isNewChild = function _isNewChild(item, cb) {
  ...
  that._log.debug('tree _isNewChild check if this version already exists %j', item.h);
  ...
  this._db.get(vKey, function(err, dsKey) {
    ...
    if (!err) {
      ...
      if (key.id !== item.h.id) {
        ...
        that._log.info('tree _isNewChild %j %s %s', error, key.id, item.h.v);
        cb(error);
        return;
      } else {
        // the version does already exist
        that._log.debug('tree _isNewChild version already exists %s', item.h.v);
        cb(null, false);
        return;
      }
    }
    ...
    async.every(item.h.pa, function(pa, cb2) {
      ...
      that._log.info('tree _isNewChild descend %s search for parent %s', item.h.id, pa);
      ...
      it.on('data', function(sItem) {
        ...
        that._log.debug('tree _isNewChild descend item %j', sItem.h);
        ...
        if (sItem.h.v === pa) {
          ...
          that._log.debug('tree _isNewChild version %s does not equal parent %s', sItem.h.v, pa);
        }
      });
      ...
      if (!parentsExist) {
        that._log.info('tree _isNewChild not all parents exist %j', item.h);
        cb(null, false);
        return;
      }
      ...
      it.on('data', function(i) {
        ...
        that._log.debug('tree _isNewChild idxHeads %j %s', item.h, i);

        // resolve item
        it.pause();
        that._db.get(that._composeDsKey(item.h.id, i), function(err, sItem) {
          that._log.debug('tree _isNewChild resolved head %s for %j', i, item.h);
          ...
          if (!sItem.h.c && !sItem.h.d) {
            that._log.info('tree _isNewChild existing head %j for %j', sItem.h, item.h);
            // no conflict and not deleted
            // a head item for this root exists, not ok, stop searching any further
            parentFound = true;
            it.destroy();
          } else {
            that._log.debug('tree _isNewChild existing head was _c or _d %j continue...', sItem.h);
            it.resume();
          }
        });
      });
      ...
      it.on('close', function() {
        ...
        if (parentFound) {
          that._log.info('tree _isNewChild active parent exists for new root item %j', item.h);
          cb(null, false);
          return;
        }

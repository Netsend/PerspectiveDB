 ...
 * @param {String} name  name of this tree
...
function Tree(db, name, opts) {
  ...
  if (name.search('\x00') !== -1) { throw new TypeError('name must not contain 0x00'); }
  if (name.search('\xff') !== -1) { throw new TypeError('name must not contain 0xff'); }
  ...
  // partition db in data and indexes
  this._db = db;
  // data store, where _h.id + _h.i is the key and the object is the value
  this._dbDataPrefix = new Buffer('data\x00' + name);

  // open indexes
  // i index where _h.i is the key and _h.id + _h.v is the value
  // i is a 48-bit monotonically increasing number saved in a 6-byte Buffer
  this._idxIPrefix = new Buffer('idx/i\x00' + name);

  // head index where _h.id + _h.v is the key and _h.i is the value
  this._idxHeadPrefix = new Buffer('idx/heads\x00' + name);
...
util.inherits(Tree, Duplex);
module.exports = Tree;

/**
 * Convert a 48-bit unsigned LE buffer to a number.
 *
 * @params {Buffer} b  buffer containing the number
 * @return {Number} number from the buffer
 */
Tree.bton = function bton(b) {
  if (!Buffer.isBuffer(b)) {
    throw new Error('b must be a buffer');
  }
...
/**
 * Convert a number to a 48-bit unsigned LE buffer.
 *
 * @params {Number} n  number for the buffer
 * @params {Number} size  size to allocate
 * @return {Buffer} size byte buffer containing i
 */
Tree.ntob = function ntob(n, size) {
  if (typeof n !== 'number') { throw new Error('n must be a number'); }
  if (typeof size !== 'number') { throw new Error('size must be a number'); }

  var b = new Buffer(size);
  b.writeUIntLE(n, 0, size);
  return b;
};
...
/**
 * Implementation of _read method of Duplex stream. This method is not called
 * directly.
 */
//VersionedCollection.prototype._read = function(item, encoding, cb) {
//  this._saveItem(item, cb);
//};

/**
 * Save a new version in the appropriate DAG if it connects.
 *
 * Implementation of _write method of Duplex stream. This method is not called
 * directly.
 *
...
Tree.prototype._write = function(item, encoding, cb) {
  ...
  this._validParents(item, function(err, valid) {
...
Tree._composeKey = function _composeKey(prefix, id, n, size) {
  ...
  if (typeof id === 'string') {
    idlen = Buffer.byteLength(id);
  }

  if (!idlen) { throw new Error('id must be a buffer or (convertible to) a string'); }
  if (typeof size !== 'number') { throw new Error('n must be a number'); }

  // then create and fill the new buffer
  var b = new Buffer(prefix.length + 1 + idlen + 1 + size);

  prefix.copy(b);
  b[prefix.length] = 0x00;

  if (typeof id === 'string') {
    b.write(id, prefix.length + 1);
  } else { // it must be a buffer
    id.copy(b, prefix.length + 1);
  }

  b[prefix.length + 1 + idlen] = 0x00;

  // write n
  if (Buffer.isBuffer(n)) {
    n.copy(b, prefix.length + 1 + idlen + 1);
  } else {
    b.writeUIntLE(n, prefix.length + 1 + idlen + 1, size);
  }
...
Tree.prototype._getDataStoreKey = function _getDataStoreKey(header) {
  return Tree._composeKey(this._dbDataPrefix, header.id, header.i, this._iSize);
};
...
Tree.prototype._validParents = function _validParents(item, cb) {
  ...
  async.every(item._h.pa, function(pa, cb2) {
    ...
    //var it = that._db.createValueStream({ reverse: true, gte: this._dbDataPrefix + '\x00' + item._h.id, lt: this._dbDataPrefix + '\x00' + item._h.id + '\xff' });
    var r = Tree.getRange(this._dbDataPrefix, item._h.id);
    var it = that._db.createValueStream({ reverse: true, gte: r.s, lt: r.e });

    var parentFound = false;

    it.on('data', function(sItem) {
      that._log.info('vc _validParents dbData descending %j', sItem._h);
      if (sItem._h._v === pa) {
        // great success
        parentFound = true;
        it.destroy();
    ...
    //var it = that._db.createValueStream({ reverse: true, gte: item._h.id, lt: this._idxHeadPrefix + '\x00' + item._h.id + '\xff' });
    var r = Tree.getRange(this._idxHeadPrefix, item._h.id);
    var it = that._db.createValueStream({ reverse: true, gte: r.s, lt: r.e });
    ...
    it.on('data', function(i) {
      ...
      // resolve item
      it.pause();
      that._db.get(Tree._getDataStoreKey({ id: item._h.id, i: i }), function(err, sItem) {
        that._log.debug('vc _validParents resolved head %s for %j', i, item._h);

        if (err) {
          that._log.err('vc _validParents resolve head %s for %j failed', i, item._h);
          error = err;
          it.destroy();
          return;
        }

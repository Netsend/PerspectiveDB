...
Tree.prototype.setConflictByVersion = function setConflictByVersion(version, cb) {
  ...
  this.getByVersion(version, function(err, item) {
    ...
    that._db.put(dsKey, BSON.serialize(item), function(err) {
      if (err) { cb(err); return; }
    ...
    if (parents.length) {
      ...
      async.each(parents, function(pa, cb2) {
        ...
        that._db.get(that._composeVKey(pa), function(err, dsKey) {
          ...
          // TODO: make sure the parent is not deleted
      ...
      var it = that._db.createReadStream({ gte: r.s, lt: r.e });

      var nonDeletedHeads = {};

      it.on('error', cb);

      it.on('data', function(head) {
        var parsedHeadKey = Tree.parseKey(head.key, { decodeV: 'base64' });
        var parsedHeadVal = Tree.parseHeadVal(head.value);

        if (!parsedHeadVal.d) {
          that._log.info('t:%s _validNewItem non-deleted head exists in tree %j for %j', that.name, parsedHeadKey.v, item.h);
          nonDeletedHeads[parsedHeadKey.v] = true;
        }
      });

      it.on('end', function() {
        // further check batch for new heads up to this item
        batch.some(function(nitem) {
          nitem = nitem.chunk;
          if (nitem.h.id !== item.h.id) {
            // different DAG, skip
            return false;
          }
          if (nitem.h.v === item.h.v) {
            return true;
          }
          // ensure existing head is replaced
          nitem.h.pa.forEach(function(pa) {
            that._log.info('t:%s _validNewItem previous head %s deleted in batch by %s', that.name, pa, nitem.h.v);

            delete nonDeletedHeads[pa];
          });
          if (!nitem.h.d) {
            nonDeletedHeads[nitem.h.v] = true;
          }
          return false;
        });

        var heads = Object.keys(nonDeletedHeads);
        if (heads.length) {
          that._log.info('t:%s _validNewItem head(s) in DAG and batch are not all deleted, required for new root item %j, existing heads: %s', that.name, item.h, heads);
          cb(null, false, [], false);
          return;
        }

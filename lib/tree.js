...
function Tree(db, name, opts) {
  ...
  this._idxPePrefix   = Tree.getPrefix(name, PEKEY);
 ...
 *                     will be returned.
 *   decodePe {String}  decode pe as string with given encoding. Encoding must
 *                      be either "hex", "utf8" or "base64". If not given a
...
Tree.parseKey = function parseKey(key, opts) {
  ...
  case PEKEY:
    nlen = key[offset++];
    if (!(nlen > 0)) { throw new Error('pe must be at least one byte'); }
    ret.pe = key.slice(offset, offset += nlen);
    if (key[offset++] !== 0x00) { throw new Error('expected a null byte'); }
    if (ret.pe && peDecode) { ret.pe = ret.pe.toString(peDecode); }
...
/**
 * Parse pe index value.
 *
 * @param {Buffer} value  value of a peKey
 ...
 * @return {Object} containing the version
 */
Tree.parsePeVal = function parsePeVal(value, decodeV) {
  if (!Buffer.isBuffer(value)) { throw new TypeError('value must be a buffer'); }
  ...
  nlen = value[offset++];
  if (!(nlen > 0)) { throw new Error('v must be at least one byte'); }
  ret.v = decodeV ? value.slice(offset).toString(decodeV) : value.readUIntBE(offset, nlen);
  offset += nlen;
 ...
 *
 * opts:
 *   id {Buffer}  id to prefix start and end
...
Tree.prototype.getHeads = function getHeads(opts, iterator, cb) {
  ...
  if (opts.id && !Buffer.isBuffer(opts.id)) { throw new TypeError('opts.id must be a buffer'); }
  if (typeof opts.skipConflicts !== 'undefined' && typeof opts.skipConflicts !== 'boolean') { throw new TypeError('opts.skipConflicts must be a boolean'); }
  ...
  var r = this.getHeadKeyRange(opts.id);
  ...
  it.on('data', function(headIdx) {
    ...
    that._db.get(that._composeDsKey(key.id, val.i), function(err, item) {
      ...
      iterator(opts.raw ? item : BSON.deserialize(item), function(err) {
 ...
 */
Tree.prototype.getByVersion = function getByVersion(version, cb) {
  if (typeof version !== 'number' && typeof version !== 'string') { throw new TypeError('version must be a number or a base64 string'); }
  if (typeof cb !== 'function') { throw new TypeError('cb must be a function'); }
  ...
  var that = this;
  this.getDsKeyByVersion(version, function(err, dsKey) {
 ...
 *
 * opts:
 *   i {Number}  offset, valid lbeint (mutually exclusive with opts.v)
 *   v {Number|base64 String}  offset, valid lbeint or base64 int (mutually
 *                             exclusive with opts.i)
 *   raw {Boolean, default false}  whether to return a BSON serialized or
 *                                 deserialezed object (false).
 *   excludeOffset {Boolean, default false}  whether to return the i or v item
 *                                           or not
 */
Tree.prototype.iterateInsertionOrder = function iterateInsertionOrder(opts, iterator, cb) {
  ...
  if (typeof opts.i !== 'undefined' && typeof opts.i !== 'number') { throw new TypeError('opts.i must be a number'); }
  if (typeof opts.v !== 'undefined') {
    if (typeof opts.v !== 'number' && typeof opts.v !== 'string') { throw new TypeError('opts.v must be a base64 string or a number'); }
    if (typeof opts.v === 'string' && Buffer.byteLength(opts.v, 'base64') !== this._vSize) { throw new Error('opts.v must be the same size as the configured vSize'); }
  }
  if (opts.i && opts.v) { throw new Error('opts.i and opts.v are mutually exclusive'); }
  if (typeof opts.raw !== 'undefined' && typeof opts.raw !== 'boolean') { throw new TypeError('opts.raw must be a boolean'); }
  if (typeof opts.excludeOffset !== 'undefined' && typeof opts.excludeOffset !== 'boolean') { throw new TypeError('opts.excludeOffset must be a boolean'); }

  var that = this;

  var offset = opts.i;

  // ensure async even without opts.v
  ...
  if (opts.v) {
    ...
    tasks.push(function(cb2) {
      var vKey = that._composeVKey(opts.v);
      that._log.debug('tree iterateInsertionOrder resolve opts.v %s', opts.v);

      that._db.get(vKey, function(err, dsKey) {
        if (err) {
          that._log.err('tree iterateInsertionOrder %s', err);
          cb2(err);
          return;
        }

        var i = Tree.parseKey(dsKey).i;
        that._log.debug('tree iterateInsertionOrder resolved opts.v to i %s', i);
        offset = i;
        cb2();
      });
    });
  ...
  async.series(tasks, function(err) {
    ...
    var r = that.getIKeyRange({ minI: offset });
    var it;
    if (opts.excludeOffset) {
      it = that._db.createReadStream({ gt: r.s, lte: r.e });
    } else {
      it = that._db.createReadStream({ gte: r.s, lte: r.e });
    }

    that._log.info('tree iterateInsertionOrder ikey range %j', r);
    ...
    it.on('data', function(obj) {
      ...
      that._db.get(dsKey, function(err, item) {
        ...
        that._log.debug('tree iterateInsertionOrder item push %j', item);

        iterator(opts.raw ? item : BSON.deserialize(item), function(err, cont) {
          if (err) {
            error = err;
            paused = false;
            closer ? closer(error) : it.destroy();
            return;
          }
          if (typeof cont !== 'undefined' && !cont) {
            paused = false;
            closer ? closer() : it.destroy();
            return;
          }
          paused = false;
          closer ? closer() : it.resume();
        });
      });
    });
...
Tree.prototype.lastByPerspective = function lastByPerspective(pe, decodeV, cb) {
  ...
  it.on('error', function(err) {
    that._log.err('tree lastByPerspective %j', err);
    error = err;
  });

  it.on('data', function(val) {
    var v = Tree.parsePeVal(val, decodeV).v;
    that._log.debug('tree lastByPerspective v %s', v);
    found = v;
  });

  it.on('close', function() {
    that._log.debug('tree lastByPerspective %s', found);
    cb(error, found);
  });
};
 ...
 */
Tree.prototype.maxI = function maxI(cb) {
  if (typeof cb !== 'function') { throw new TypeError('cb must be a function'); }
  ...
  it.on('error', function(err) {
    that._log.err('tree maxI %j', err);
    error = err;
  });

  it.on('data', function(i) {
    that._log.debug('tree maxI i %s', i);
    found = Tree.parseKey(i).i;
  });

  it.on('close', function() {
    that._log.debug('tree maxI %s', found);
    cb(error, found);
  });
};
...
/**
 * Save a multiple new versions in the appropriate DAG if they connect.
 *
...
Tree.prototype._writev = function(items, cb) {
  ...
  async.each(items, function(chunk, cb2) {
    ...
    async.eachSeries(items, function(chunk, cb2) {
      var item = chunk.chunk;
      that._nextI(function(err, i) {
        ...
        var peKey = that._composePeKey(item.h.pe || that.name, i);
        var peVal = that._composePeVal(item.h.v);
        ...
        tra.push({ type: 'put', key: peKey, value: peVal });
...
/**
 * Get the key for the pe index.
 *
 * @param {Buffer|String} pe  perspective that is a string or a buffer
 * @param {Number} i  valid lbeint
 * @return {Buffer} valid key of subtype pekey
 */
Tree.prototype._composePeKey = function _composePeKey(pe, i) {
  if (!Buffer.isBuffer(pe)) {
  ...
  var b = new Buffer(this._idxPePrefix.length + 1 + pe.length + 1 + 1 + this._iSize);
  var offset = 0;

  this._idxPePrefix.copy(b, offset);
  offset += this._idxPePrefix.length;
...
/**
 * Get the value for an item in the pe index.
 *
 * @param {String} v  base64 representation of a lbeint
 * @return {Buffer} valid pe index value
 */
Tree.prototype._composePeVal = function _composePeVal(v) {
  if (typeof v !== 'string') { throw new TypeError('v must be a string'); }

  if (v.length * 6 !== this._vSize * 8) {
    this._log.err('tree _composePeVal base64 version length is %s bits instead of %s bits (vSize)', v.length * 6, this._vSize * 8);
    throw new Error('v is too short or too long');
  }

  var b = new Buffer(1 + this._vSize);
  var offset = 0;
...
/**
 * Get a range object with start and end points for the pekey index.
 *
 * @param {Buffer} [pe]  optional pe to prefix start and end
 * @return {Object}  start and end buffer: { s: buffer, e: buffer }
 */
Tree.prototype.getPeKeyRange = function getPeKeyRange(pe) {
  if (pe && !Buffer.isBuffer(pe)) { throw new TypeError('pe must be a buffer if provided'); }

  var prefix = this._idxPePrefix;
...
Tree.prototype._isConnected = function _isConnected(item, cb) {
  ...
  this._db.get(vKey, function(err, dsKey) {
    ...
    async.every(item.h.pa, function(pa, cb2) {
      ...
      var r = that.getDsKeyRange(Tree._toBuffer(item.h.id));
      ...
      it.on('data', function(i) {
        ...
        that._db.get(that._composeDsKey(item.h.id, i), function(err, sItem) {
          ...
          if (!sItem.h.c && !sItem.h.d) {
            ...
            that._log.debug('tree _isConnected existing head was _c or _d %j continue...', sItem.h);
            it.resume();
          }
        });
      });

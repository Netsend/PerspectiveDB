...
Tree.prototype.getHeads = function getHeads(opts, iterator, cb) {
  ...
  var r = this.getHeadKeyRange(id);
  this._log.info('t:%s getHeads range %j', this.name, r);
  ...
  it.on('data', function(headIdx) {
    ...
    that._log.debug('t:%s getHeads iterate key %j val %j', that.name, key, val);
    ...
    that._db.get(that._composeDsKey(key.id, val.i), function(err, item) {
      ...
      that._log.debug('t:%s getHeads iterate item %j', that.name, item);
 ...
 *
 * @param {Object} [opts]  object containing configurable parameters
 * @param {Function} iterator  function (item, next) called with each item. Call
 *                             next when done handling the item.
 *                             next(err, continue) where continue defaults to
 *                             true.
 * @param {Function} cb  First parameter will be an error object or null.
 *
 * opts:
...
Tree.prototype.createReadStream = function createReadStream(opts) {
  ...
  function setupNewReaderToWriter() {
    that._log.debug('t:%s createReadStream open reader', that.name);
...
Tree.prototype.lastVersion = function lastVersion(decodeV, cb) {
  ...
  it.on('data', function(val) {
    that._log.debug('t:%s lastVersion i %s', that.name, val);
...
Tree.prototype.lastByPerspective = function lastByPerspective(pe, decodeV, cb) {
  ...
  this._db.get(usKey, function(err, vkey) {
    if (err) {
      if (err.notFound) {
        that._log.info('t:%s lastByPerspective lookup not found %j', that.name, usKey);
        cb(null, null);
        return;
      } else {
        that._log.err('t:%s lastByPerspective lookup error %j', that.name, err);
        cb(err);
        return;
      }
    }
    ...
    that._log.debug('t:%s lastByPerspective %s', that.name, v);
    cb(null, v);
  });
};
...
Tree.prototype._getDsKeyByVersion = function _getDsKeyByVersion(version, cb) {
  ...
  this._log.debug('t:%s _getDsKeyByVersion %s', this.name, version);
...
Tree.prototype._maxI = function _maxI(cb) {
  ...
  it.on('data', function(i) {
    that._log.debug('t:%s _maxI i %s', that.name, i);
    found = Tree.parseKey(i).i;
  });

  it.on('end', function() {
    that._log.debug('t:%s _maxI %s', that.name, found);
    cb(error, found);
  });
};
  ...
  async.filterSeries(items, function(chunk, cb2) {
    ...
    async.eachSeries(fitems, function(chunk, cb2) {
      ...
      that._nextI(function(err, i) {
        ...
        that._log.debug('t:%s _writev iKey: %s, vKey: %s, headKey: %s, dsKey: %s', that.name, iKey.toString('hex'), vKey.toString('hex'), headKey.toString('hex'), dsKey.toString('hex'));
      ...
      that._db.batch(tra, function(err) {
        ...
        that._log.info('t:%s _writev items written %d', that.name, tra.length);
        cb();
      });
    });
  });
...
Tree.prototype._composeHeadKey = function _composeHeadKey(id, v) {
  ...
  this._log.debug('t:%s _composeHeadKey %s %s', this.name, id, v);
...
Tree.prototype._nextI = function _nextI(cb) {
  ...
  this._log.debug('t:%s _nextI first time, resolve max i', that.name);

  // else find the last increment number
  this._maxI(function(err, found) {
    that._log.debug('t:%s _nextI close, max i found %s', that.name, found);
    if (err) { cb(err); return; }

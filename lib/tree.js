 ...
 * number.
 *
 * @param {Buffer} b  buffer that contains an idxIKey (prefix + i)
 ...
 * @return {Buffer} valid key of subtype ikey
 */
 ...
 * value for id and v. A valid id means a buffer, a string or an object that
 ...
 */
Tree.prototype._getHeadKey = function _getHeadKey(id, v) {
  id = Tree._ensureString(id);
  if (typeof v !== 'string') { throw new TypeError('v must be a string'); }

  if (v.length * 6 !== this._vSize * 8) {
    this._log.err('tree _getHeadKey base64 version length is %s bits instead of %s bits (vSize)', v.length * 6, this._vSize * 8);
    throw new Error('v is too short or too long');
  }
  this._log.debug('tree _getHeadKey %s %s %s', v.length * 6, this._vSize * 8, v);
 ...
 *
 * @param {Object} header  valid header (item._h)

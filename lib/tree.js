  ...
  this._db.get(vKey, function(err, dsKey) {
    ...
    if (parents.length) {
      ...
      it.on('data', function(headVal) {
        if (Tree.parseHeadVal(headVal).d) {
          that._log.debug('tree _validNewItem missingParents head deleted %j continue...', headVal.h);
        } else {
          that._log.info('tree _validNewItem non-deleted head exists %j for %j', headVal.h, item.h);
          // not deleted

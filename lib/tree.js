...
Tree.prototype.iterateInsertionOrder = function iterateInsertionOrder(opts, iterator, cb) {
  ...
  if (opts.v) {
    ...
    tasks.push(function(cb2) {
      ...
      that._log.debug('tree createReadStream resolve opts.v %s', opts.v);
      ...
      that._db.get(vKey, function(err, dsKey) {
        ...
        that._log.debug('tree createReadStream resolved opts.v to i %s', key.i);
        offset = key.i;
        cb2();
      });
    });
  ...
  async.series(tasks, function(err) {
    ...
    that._log.info('tree createReadStream ikey range %j', r);
    ...
    it.on('data', function(obj) {
      ...
      that._db.get(dsKey, function(err, item) {
        ...
        that._log.debug('tree createReadStream item push %j', item);
  ...
  async.each(items, function(chunk, cb2) {
    ...
    async.eachSeries(items, function(chunk, cb2) {
      ...
      that._db.batch(tra, function(err) {
        ...
        if (err) {
          that._log.err('tree _writev items not written %j %s', tra, err);
          cb(err);
          return;
        }

        that._log.info('tree _writev items written %s %j', tra);
        cb();
      });
    });
  });

 ...
 */
Tree.prototype.minI = function minI(cb) {
  if (typeof cb !== 'function') { throw new TypeError('cb must be a function'); }

  // find the last increment number
  var r = this.getIKeyRange();
  var it = this._db.createKeyStream({ gt: r.s, lt: r.e, limit: 1 });
  ...
  it.on('error', function(err) {
    that._log.err('tree minI %j', err);
    error = err;
  });

  it.on('data', function(i) {
    that._log.debug('tree minI i %s', i);
    found = Tree.parseKey(i).i;
  });

  it.on('close', function() {
    that._log.debug('tree minI %s', found);
    cb(error, found);
  });
};

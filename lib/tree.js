...
Tree.prototype.getDsKeyRange = function getDsKeyRange(opts) {
  ...
  var id = opts.id;
  if (typeof id !== 'undefined' && !Buffer.isBuffer(id)) {
  ...
  if (id) {
    ...
    if (minI) {
      slen += 1 + 1 + this._iSize;
    }

    if (maxI) {
      elen += 1 + 1 + this._iSize;
    }
  }
  ...
  if (id) {
    ...
    if (minI) {
      s[offset] = 0x00;
      s[offset + 1] = this._iSize;
      s.writeUIntBE(minI, offset + 2, this._iSize);
    }

    if (maxI) {
      e[offset] = 0x00;
      e[offset + 1] = this._iSize;
      e.writeUIntBE(maxI, offset + 2, this._iSize);
      offset += 2;
      offset += this._iSize;
    }
  } else {
 ...
 *   v {Number|base64 String}  offset, valid lbeint or base64 int
 *   reverse {Boolean, default false}  if true, starts with the children
 *   raw {Boolean, default false}  whether to return a BSON serialized or
 *                                 deserialezed object (false).
...
Tree.prototype.iterateInsertionOrder = function iterateInsertionOrder(opts, iterator, cb) {
  ...
  if (typeof opts.reverse !== 'undefined' && typeof opts.reverse !== 'boolean') { throw new TypeError('opts.reverse must be a boolean'); }
  if (typeof opts.raw !== 'undefined' && typeof opts.raw !== 'boolean') { throw new TypeError('opts.raw must be a boolean'); }
  ...
  async.series(tasks, function(err) {
    ...
    if (id) {
      ...
      r = that.getDsKeyRange({ id: id, minI: offset });
    } else {
      // start reading the ikey index
      r = that.getIKeyRange({ minI: offset });
    }

    if (opts.excludeOffset) {
      if (opts.reverse) {
        it = that._db.createReadStream({ gte: r.s, lt: r.e, reverse: opts.reverse });
      } else {
        it = that._db.createReadStream({ gt: r.s, lte: r.e, reverse: opts.reverse });
      }
    } else {
      it = that._db.createReadStream({ gte: r.s, lte: r.e, reverse: opts.reverse });
    }

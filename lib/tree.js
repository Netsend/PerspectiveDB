...
Tree.prototype.iterateInsertionOrder = function iterateInsertionOrder(opts, iterator, cb) {
  ...
  if (opts.last != null) {
    ...
    if (typeof opts.last === 'string' && Buffer.byteLength(opts.last, 'base64') !== this._vSize) { throw new Error('opts.last must be the same size as the configured vSize'); }
  }
  ...
  // ensure async even if not resolving a first or last version
  ...
  if (opts.first) {
    ...
    tasks.push(function(cb2) {
      var vKey = that._composeVKey(opts.first);
      that._log.debug('tree iterateInsertionOrder resolve opts.first %s', opts.first);
      ...
      that._db.get(vKey, function(err, dsKey) {
        ...
        that._log.debug('tree iterateInsertionOrder resolved opts.first to i %s', i);
        first = i;
        cb2();
      });
    });
  ...
  if (opts.last) {
    // resolve v to i
    tasks.push(function(cb2) {
      var vKey = that._composeVKey(opts.last);
      that._log.debug('tree iterateInsertionOrder resolve opts.last %s', opts.last);
      ...
      that._db.get(vKey, function(err, dsKey) {
        ...
        var i = Tree.parseKey(dsKey).i;
        that._log.debug('tree iterateInsertionOrder resolved opts.last to i %s', i);
        last = i;
        cb2();
      });
    });
  ...
  async.series(tasks, function(err) {
    ...
    if (id) {
      ...
      r = that.getDsKeyRange({ id: id, minI: first, maxI: last});
    } else {
      // start reading the ikey index
      r = that.getIKeyRange({ minI: first, maxI: last });
    }
    ...
    that._log.info('tree iterateInsertionOrder streamOpts %j', streamOpts);

    it = that._db.createReadStream(streamOpts);

...
Tree.prototype.getHeads = function getHeads(opts, iterator, cb) {
  ...
  this._log.debug('t:%s getHeads', this.name);
...
Tree.prototype.iterateInsertionOrder = function iterateInsertionOrder(opts, iterator, cb) {
  ...
  var transformer = new Transform({
    objectMode: true,
    transform: function(obj, enc, cb2) {
      iterator(obj, function(err, cont) {
        if (err) {
          // end this transform
  ...
  (new StreamTree(this, opts)).pipe(transformer);
};
  ...
  async.filterSeries(items, function(chunk, cb3) {
    ...
    async.eachSeries(fitems, function(chunk, cb3) {
      ...
      that._nextI(function(err, i) {
        ...
        that._log.debug('t:%s _writev new data item %j', that.name, item.h);

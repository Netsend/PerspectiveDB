...
/**
 * Get the key for the data store. It is assumed that pe is a string or an
 * object that implements the "toString" method. A valid i is a lbeint.
 *
 * @param {Buffer|String|Object} pe  perspective that is a buffer, string or
 *                                   implements "toString"
 * @param {Number} i  valid lbeint
...
Tree.prototype._composePeKey = function _composePeKey(pe, i) {
  if (!Buffer.isBuffer(pe)) {
    pe = new Buffer(Tree._ensureString(pe));
  }
  if (typeof i !== 'number') { throw new TypeError('i must be a number'); }

...
Tree.prototype._write = function(item, encoding, cb) {
  ...
  this._isNewChild(item, function(err, isNew) {
    ...
    if (!isNew) {
      ...
      that._log.err('tree _write %s %j', error, item.h);
      cb(error);
      return;
    }
    ...
    that._nextI(function(err, i) {
      ...
      item.h.pa.forEach(function(pa) {
        that._log.debug('tree _write delete old head', item.h.id, pa);
      ...
      that._db.batch(tra, function(err) {
        ...
        if (err) {
          that._log.err('tree _write item not written %s %j %j', i, item.h, err);
          cb(err);
          return;
        }

        that._log.info('tree _write item written %s %j', i, item.h);
        cb();
      });
    });
  });

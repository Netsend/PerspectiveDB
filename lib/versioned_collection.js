 ...
 */
VersionedCollection.prototype.saveCollectionItem = function saveCollectionItem(item, pa, cb) {
    ...
    that._snapshotCollection.findOne({ '_id._id': item._id, '_id._pe': that.localPerspective }, { sort: { '_id._i': -1 } }, function(err, snapshotItem) {
      ...
      if (!snapshotItem) {
        that.saveCollectionItem(item, [], function(err, newObj) {
          ...
          // set ackd
          that._setAckd(newObj._id._id, newObj._id._v, newObj._id._pe, function(err) {
            if (err) { error = err; stream.destroy(); return; }
      ...
      if (!snapshotItem._m3._ack) {
        ...
        that._setAckd(snapshotItem._id._id, snapshotItem._id._v, snapshotItem._id._pe, function(err) {
          if (err) { error = err; stream.destroy(); return; }
 ...
 * @param {String} remote  name of the remote, should differ from local perspective
 *                         and match item.
 ...
 */
VersionedCollection.prototype.saveRemoteItem = function saveRemoteItem(item, remote, afterCb, cb) {
  if (typeof item !== 'object') { throw new TypeError('item must be an object'); }
  if (typeof remote !== 'string') { throw new TypeError('remote must be a string'); }
  ...
  var that = this;
  if (remote === that.localPerspective) {
  ...
  var queue = that._remoteQueues[remote];
  var error;

  if (!queue) {
    process.nextTick(function() {
      error = 'missing remote queue, pass remote to constructor';
      if (!that._hide) { console.error(that.databaseName, that.collectionName, 'saveRemoteItem', error, remote, that._remotes); }
      cb(new Error(error));
    });
    return;
  }

  // check perspective and other properties
  if (item._id._pe !== remote) {
    process.nextTick(function() {
      error = 'item perspective does not equal remote';
      if (!that._hide) { console.error(that.databaseName, that.collectionName, 'saveRemoteItem', error, item, remote); }
      cb(new Error(error));
    });
    return;
  }
  ...
  if (this._queueLimit <= queue.length) {
    ...
    setTimeout(function() {
      ...
      that.saveRemoteItem(item, remote, afterCb, cb);
 ...
 *                       null.
 */
VersionedCollection.prototype._setAckd = function _setAckd(id, v, pe, cb) {
  ...
  var that = this;
  that._snapshotCollection.update({ '_id._id': id, '_id._v': v, '_id._pe': pe }, { $set: { '_m3._ack': true }}, {w: 1, comment: '_setAckd'}, function(err, updated) {
    if (err) {
...
VersionedCollection.prototype._merge = function _merge(objX, objY, cb) {
  ...
  // do not alter original objects, create shallow clones of the objects and it's _ids
  var itemX = {}, itemY = {};
  ...
  function done(err, nlcas, perspectives) {
    ...
    if (!nlcas.length) {
      ...
      return cb(new Error('no lca found'));
    }
    if (!perspectives.length) {
      if (!that._hide) { console.error(that.databaseName, that.collectionName, '_merge no perspectives', debugObj); }
      return cb(new Error('no perspectives'));
    }

    // prevent side-effects, shallow clone lca's + deep clone _id and _m3
    var lcas = [];
    nlcas.forEach(function(lca) {
      ...
      if (lca._m3) {
        Object.keys(lca._m3).forEach(function(key) {
          obj._m3[key] = lca._m3[key];
        });
      }
      lcas.push(obj);
    });
    ...
    if (effectiveNumberOfLCAs === 1) {
      // first reset _id and remove _m3 to prevent incorrect conflicts on any future merge
      // save a reference of the id's in case a fast-forward by merge is done.
      var lcaIds = [];
      lcas.forEach(function(lca, i) {
        lcaIds[i] = lca._id;
        lca._id = lca._id._id;
        // make sure _ack is never set
        if (lca._m3) { delete lca._m3._ack; }
      });

      var itemXid = itemX._id;
      itemX._id = itemX._id._id;
      // make sure _ack is never set
      if (itemX._m3) { delete itemX._m3._ack; }

      var itemYid = itemY._id;
      itemY._id = itemY._id._id;
      // make sure _ack is never set
      if (itemY._m3) { delete itemY._m3._ack; }
      ...
      if (perspectives.length === 1) {
        ...
        if (itemXid._v && itemXid._v === itemYid._v) { return cb(null, [objX], perspectives); }

        // if lca equals one item, fast-forward to the other item
        if (lcaIds[0]._v && lcaIds[0]._v === itemXid._v) { return cb(null, [objY], perspectives); } // ff to original objY
        if (lcaIds[0]._v && lcaIds[0]._v === itemYid._v) { return cb(null, [objX], perspectives); } // ff to original objX
        ...
        if (Array.isArray(merge)) {
          ...
          return cb(new Error('merge conflict'), merge, perspectives);
        }
        ...
        return cb(null, [merge], perspectives);
      }
      ...
      if (perspectives.length === 2) {
        ...
        if (lcaIds[0]._v && itemXid._v === lcaIds[0]._v) {
          ...
          if (Array.isArray(merged[0])) {
            ...
            return cb(new Error('merge conflict'), merged, perspectives);
          }
          ...
          return cb(null, merged, perspectives);
        }
        if (lcaIds[0]._v && itemYid._v === lcaIds[0]._v) {
  ...
  that._findLCAs(itemX, itemY, function(err, lcas) {
    ...
    if (lcas.length < 1) {
      ...
      return cb(new Error('no lca found'), debugObj);
    }
    ...
    that._snapshotCollection.find(selector, { comment: '_merge' }).toArray(function(err, lcaPerspectives) {
      ...
      if (err) { return cb(err, debugObj, perspectives); }
      if (lcaPerspectives.length !== lcas.length * perspectives.length) {
        ...
        return cb(new Error('error when fetching perspective bound lca\'s'), debugObj, perspectives);
      }
...
VersionedCollection.prototype._applyOplogUpdateFullDoc = function _applyOplogUpdateFullDoc(oplogItem, cb) {
  ...
  this._snapshotCollection.findOne(selector, { comment: '_applyOplogUpdateFullDoc' }, function(err, item) {
    ...
    if (item && that.compareDAGItemWithCollectionItem(item, oplogItem.o)) {
      ...
      // set ackd
      that._setAckd(item._id._id, item._id._v, item._id._pe, function(err) {
        if (err) {
      ...
      that._findLastAckdOrLocallyCreated(oplogItem.o._id, function(err, item2) {
        ...
        if (oplogItem.op === 'i' && !item2) {
          ...
          that.saveCollectionItem(oplogItem.o, [], function(err, newObj) {
            if (err) { return cb(err, oplogItem); }
            cb(null, newObj);
          });
          return;

 ...
 *                                           queue at one time
 *   log {Object, default console}  log object that contains debug, info, notice,
 *       warning, err, crit and emerg functions. Uses console.log and console.error
 *       by default.
 *
...
VersionedCollection.prototype.startAutoProcessing = function startAutoProcessing(interval) {
  ...
  this._log.info('startAutoProcessing', interval);
  ...
  this._autoProcessor = setInterval(function() {
    if (!that._locked) {
      that.processQueues(function(err) {
        if (err) {
          that._log.err('startAutoProcessing processor', err);
          console.trace();
          throw err;
        }
      });
    } else {
      that._log.info('startAutoProcessing locked skip iteration');
    }
  }, interval);
};
...
VersionedCollection.prototype.stopAutoProcessing = function stopAutoProcessing(cb) {
  this._log.info('stopAutoProcessing');
...
VersionedCollection.prototype.allHeads = function allHeads(iterator, cb) {
  ...
  this._snapshotCollection.distinct('_id._id', function(err, ids) {
    if (err) {
      that._log.err('allHeads', err);
      cb(err);
      return;
    }

    that._log.info('allHeads ids to inspect', ids.length);

    // foreach id, find all open heads by following the complete DAG.
    var count = 0;
    async.eachSeries(ids, function(id, cb2) {
      count++;
      that._log.debug(count, id);
...
VersionedCollection.prototype.ackAncestorsAckd = function ackAncestorsAckd(cb) {
  ...
  stream.on('data', function(item) {
    ...
    that._log.info('ackAncestorsAckd', JSON.stringify(item));
    ...
    that._snapshotCollection.findOne({ '_id._id': item._id._id, '_id._pe': that.localPerspective, '_m3._ack': true }, { sort: { '_id._i': -1 } }, function(err, ackdItem) {
      if (err) {
        error = err;
        that._log.err('ackAncestorsAckd', error);
        stream.destroy();
        return;
      }
      ...
      that._snapshotCollection.update({ '_id._id': item._id._id, '_id._pe': that.localPerspective, '_id._i': { $lt: ackdItem._id._i }, '_m3._ack': { $ne: true } }, { $set: { '_m3._ack': true } }, { multi: true }, function(err, updated) {
        if (err) {
          error = err;
          that._log.err('ackAncestorsAckd update ackd', error);
          stream.destroy();
          return;
        }

        that._log.info('ackAncestorsAckd ackd', updated);
...
VersionedCollection.prototype.copyCollectionOverSnapshot = function copyCollectionOverSnapshot(cb) {
  ...
  stream.on('data', function(item) {
    ...
    that._log.debug('copyCollectionOverSnapshot item', JSON.stringify(item));

    that._snapshotCollection.findOne({ '_id._id': item._id, '_id._pe': that.localPerspective }, { sort: { '_id._i': -1 } }, function(err, snapshotItem) {
      if (err) {
        error = err;
        that._log.err('copyCollectionOverSnapshot', error);
        stream.destroy();
        return;
      }

      that._log.debug('copyCollectionOverSnapshot snapshotItem', JSON.stringify(snapshotItem));
      ...
      that._log.debug('copyCollectionOverSnapshot match');
...
VersionedCollection.prototype.lastByPerspective = function lastByPerspective(perspective, ack, cb) {
  this._log.info('lastByPerspective');
...
VersionedCollection.prototype.maxOplogPointer = function maxOplogPointer(cb) {
  this._log.info('maxOplogPointer');
...
VersionedCollection.prototype.determineRemoteOffset = function determineRemoteOffset(pe, cb) {
  this._log.info('determineRemoteOffset');
...
VersionedCollection.prototype.saveRemoteItem = function saveRemoteItem(item, afterCb, cb) {
  ...
  if (msg) {
    process.nextTick(function() {
      that._log.err(msg, JSON.stringify(item));
      cb(new Error(msg));
    });
    return;
  }
  ...
  if (!queue) {
    that._log.info('saveRemoteItem add new remote', item._id._pe);
  ...
  if (this._queueLimit <= queue.length) {
    ...
    this._log.warning('saveRemoteItem queue full, wait and retry. queue length: ', queue.length);
    setTimeout(function() {
      that._log.info('saveRemoteItem retry...', queue.length);
...
VersionedCollection.prototype.saveOplogItem = function saveOplogItem(item, afterCb, cb) {
  ...
  if (this._queueLimit <= this._oplogBuffer.length) {
    // wait with adding for timeout seconds
    this._log.warning('saveOplogItem queue full, wait and retry. queue length: ', this._oplogBuffer.length);
    setTimeout(function() {
      that._log.info('saveOplogItem retry...', that._oplogBuffer.length);
...
VersionedCollection.prototype.processQueues = function processQueues(cb) {
  this._log.debug2('processQueues');
  ...
  if (this._locked) {
    ...
    that._log.err('processQueues', error);
    process.nextTick(function() {
      cb(error);
    });
    return;
  ...
  this._log.info('processQueues start');

  this._log.info('processQueues oplog');
  that._processOplogQueue(function(err) {
    that._log.info('processQueues oplog done');
    if (err) {
      that._locked = false;
      return cb(err);
    }

    that._log.info('processQueues remote');
    that._processRemoteQueues(function(err) {
      that._log.info('processQueues remote done');
      that._locked = false;
      return cb(err);
    });
  });
...
VersionedCollection.prototype.compareDAGItemWithCollectionItem = function compareDAGItemWithCollectionItem(dagItem, collectionItem) {
  ...
  var comparator = function(key) {
    ...
    if (key === '_id') {
      ...
      that._log.debug(key, 'new item equals oplog item', eq);
      return eq;
    }
  };
...
VersionedCollection.prototype.ensureSnapshotCollection = function ensureSnapshotCollection(size, free, cb) {
  this._log.info('ensureSnapshotCollection');
  ...
  that._snapshotCollection.stats(function(err, stats) {
    if (err) {
      ...
      that._log.err('ensureSnapshotCollection', err);
      return cb(err);
    }
    ...
    if (stats.capped !== true) {
      ...
      that._log.err('ensureSnapshotCollection', error, JSON.stringify(stats));
      return cb(error);
    }
    ...
    if (stats.storageSize < size) {
      ...
      that._log.err('ensureSnapshotCollection', error, JSON.stringify(stats), size);
      return cb(error);
    }
    ...
    that._snapshotCollection.indexInformation(function(err, ixNfo) {
      ...
      if (!ixNfo || !ixNfo['_id_pe_i'] || ixNfo['_id_pe_i'].length !== 3) {
        ...
        that._log.err('ensureSnapshotCollection', error, JSON.stringify(ixNfo));
        return cb(error);
      }
...
VersionedCollection.prototype._createNewVersionByUpdateDoc = function _createNewVersionByUpdateDoc(dagItem, oplogItem, cb) {
  ...
  that._tmpCollection.insert(dagItem, {w: 1, comment: '_createNewVersionByUpdateDoc'}, function(err, inserted) {
    ...
    if (inserted.length !== 1) {
      ...
      that._log.err('_createNewVersionByUpdateDoc', error, JSON.stringify(dagItem), JSON.stringify(inserted));
      cb(error);
      return;
    }
    ...
    that._log.info('_createNewVersionByUpdateDoc selector', JSON.stringify(selector));

    that._tmpCollection.findAndModify(selector, [], oplogItem.o, {w: 0, new: true}, function(err, newObj) {
      if (err) {
        that._log.err('_createNewVersionByUpdateDoc', err);
        cb(err, oplogItem);
        return;
      }
      if (!newObj) {
        that._log.err('_createNewVersionByUpdateDoc new doc not created', JSON.stringify(dagItem), JSON.stringify(selector), JSON.stringify(oplogItem.o));
      ...
      that._tmpCollection.drop(function(err) {
        if (err) {
          that._log.err('_createNewVersionByUpdateDoc dropping _tmpCollection', err);
          cb(err, oplogItem);
          return;
        }
...
VersionedCollection.prototype.mergeAndSave = function mergeAndSave(dagItems, cb) {
  ...
  try {
    ...
    dagItems.forEach(function(dagItem) {
      try {
        ...
        that._log.err('mergeAndSave', err, JSON.stringify(first._id), JSON.stringify(dagItem._id));
        throw err;
      }
    });
  } catch(err) {
  ...
  async.reduce(dagItems, first, function(memo, item, cb2) {
    that._merge(memo, item, function(err, merged) {
      if (err) {
        that._log.err('mergeAndSave', err);
        cb2(err);
        return;
      }
      ...
      if (newObj._id._v && newObj._m3) {
        ...
        that._log.info('mergeAndSave', 'skip saved item', JSON.stringify(newObj._id));
        cb2(null);
        return;
      }
      ...
      that._getNextIncrement(function(err, i) {
        if (err) {
          that._log.err('mergeAndSave', err, i, newObj);
          cb2(err);
          return;
        }
        ...
        that._save(newObj, function(err) {
          if (err) {
            that._log.err('mergeAndSave error', err);
            cb2(err);
            return;
          }
          ...
          that._syncDAGItemWithCollection(newObj, function(err) {
            if (err) {
              that._log.err('mergeAndSave copy error', err);
              cb2(err);
              return;
            }

            that._log.info('mergeAndSave', JSON.stringify(newObj));
...
VersionedCollection.prototype._setAckd = function _setAckd(id, v, pe, op, cb) {
  ...
  that._snapshotCollection.update({ '_id._id': id, '_id._v': v, '_id._pe': pe }, { $set: { '_m3._ack': true, '_m3._op': op } }, function(err, updated) {
    ...
    if (updated !== 1) {
      ...
      that._log.err('_setAckd', error, updated, id, v, pe);
      cb(error);
      return;
    }
...
VersionedCollection.prototype._getNextIncrement = function _getNextIncrement(cb) {
  ...
  this._snapshotCollection.findOne({}, { sort: { '_id._i': -1 }, comment: '_getNextIncrement' }, function(err, item) {
    if (err) {
      that._log.err('_getNextIncrement', err);
      return cb(err);
    }
...
VersionedCollection.prototype._processRemoteQueues = function _processRemoteQueues(cb) {
  this._log.info('_processRemoteQueues');
  ...
  async.eachSeries(this._remotes, function(remote, cb2) {
    ...
    that._log.info('_processRemoteQueues remote', remote, that._remoteQueues[remote].length);
    ...
    if (err) {
      that._log.err('_processRemoteQueues error remove remote', lastRemote, err);
...
VersionedCollection.prototype._processOplogQueue = function _processOplogQueue(cb) {
  ...
  async.eachSeries(this._oplogBuffer, function(item, callback) {
    ...
    that._applyOplogItem(item.item, function(err, newItem) {
      if (err) {
        that._log.err('_processOplogQueue', err, JSON.stringify(item));
      } else {
        item.done = true;
      }
...
VersionedCollection.prototype._clearSnapshot = function _clearSnapshot(size, cb) {
  ...
  this._snapshotCollection.stats(function(err, stats) {
    if (err) {
      if (err.message !== 'ns not found' && !/^Collection .* not found/.test(err.message)) {
        that._log.err('_clearSnapshot snapshot collection stats error', err);
        return cb(err);
      }
      stats = {};
    }
    that._log.info('_clearSnapshot snapshot collection stats', JSON.stringify(stats));
    ...
    that._collection.stats(function(err, stats) {
      if (err) {
        if (err.message !== 'ns not found' && !/^Collection .* not found/.test(err.message)) {
          that._log.err('_clearSnapshot collection stats error', err);
          return cb(err);
        }
        stats = {};
      }

      that._log.info('_clearSnapshot collection stats', JSON.stringify(stats));
      ...
      that._db.dropCollection(that.snapshotCollectionName, function(err, result) {
        if (err && err.message !== 'ns not found' && !/^Collection .* not found/.test(err.message)) {
          that._log.err('_clearSnapshot drop snapshot collection error', err);
          return cb(err);
        }
        that._log.info('_clearSnapshot drop snapshot collection', result, that.snapshotCollectionName);
...
VersionedCollection.prototype._createSnapshotCollection = function _createSnapshotCollection(size, cb) {
  ...
  this._log.info('_createSnapshotCollection collection opts', JSON.stringify(opts));

  var that = this;
  this._db.createCollection(this.snapshotCollectionName, opts, function(err, snapshotCollection) {
    if (err) {
      that._log.err('_createSnapshotCollection error', err);
      return cb(err);
    }
    ...
    that._ensureIndex(function(err) {
      if (err) {
        that._log.err('_createSnapshotCollection index error', err);
        return cb(err);
      }
...
VersionedCollection.prototype._syncDAGItemWithCollection = function _syncDAGItemWithCollection(item, cb) {
  ...
  if (item._id._d) {
    this._log.info('_syncDAGItemWithCollection delete from collection', JSON.stringify(item._id));
    this._collection.remove({ _id: item._id._id }, {w: 1, comment: '_syncDAGItemWithCollection'}, cb);
  } else {
    this._log.info('_syncDAGItemWithCollection add to collection', JSON.stringify(item._id));
...
VersionedCollection.prototype._findLCAs = function _findLCAs(itemX, itemY, cb) {
  ...
  if (!itemX) {
    process.nextTick(function() {
      ...
      that._log.err('_findLCAs error', err);
      cb(err, itemX);
    });
    return;
  }
  ...
  this._log.info('_findLCAs', JSON.stringify(itemX._id), JSON.stringify(itemY._id));
  ...
  if (~['string', 'number', 'boolean'].indexOf(typeof itemX._id._id)) {
    if (itemX._id._id !== itemY._id._id) {
      process.nextTick(function() {
        ...
        that._log.err('_findLCAs error', err, itemX._id._id, itemY._id._id);
        cb(err, itemY);
      });
      return;
    }
  ...
  if (perspectives.length === 1) {
    ...
    if (lcas.length) {
      that._log.info('_findLCAs shortcut', lcas);
      process.nextTick(function() {
        cb(null, lcas);
      });
      return;
  ...
  stream.on('data', function(item) {
    ...
    that._log.debug('START HEADSX', headsX);
    that._log.debug('START HEADSY', headsY);
    that._log.debug('START ANCESTORSX', ancestorsX);
    that._log.debug('START ANCESTORSY', ancestorsY);
    that._log.debug('START LCAS', lcas);
    that._log.debug('START CAS', cas);
    ...
    that._log.debug('\nEND HEADSX', headsX);
    that._log.debug('END HEADSY', headsY);
    that._log.debug('END ANCESTORSX', ancestorsX);
    that._log.debug('END ANCESTORSY', ancestorsY);
    that._log.debug('END LCAS', lcas);
    that._log.debug('END CAS', cas);
  ...
  stream.on('close', function() {
    that._log.info('_findLCAs found', lcas);
    cb(null, lcas);
  });
...
VersionedCollection.prototype._isAncestorOf = function _isAncestorOf(version, item, cb) {
  ...
  if (version === item._id._v || ~item._id._pa.indexOf(version)) {
    this._log.info('_isAncestorOf shortcut equal version', JSON.stringify(item));
    process.nextTick(function() {
      cb(null, true);
    });
    return;
...
VersionedCollection.prototype._ensureSamePerspective = function _ensureSamePerspective(items, cb) {
  ...
  that._log.info('_ensureSamePerspective', JSON.stringify(items));
  ...
  try {
    ...
    items.forEach(function(itemNcb) {
      ...
      if (perspective !== item._id._pe) {
        that._log.err('_ensureSamePerspective perspective mismatch', perspective, item._id._pe);
...
VersionedCollection.prototype._ensureM3 = function _ensureM3(items, cb) {
  ...
  that._log.info('_ensureM3', JSON.stringify(items));
...
VersionedCollection.prototype._checkAncestry = function _checkAncestry(items, cb) {
  ...
  that._log.info('_checkAncestry', JSON.stringify(items));
    ...
    items.forEach(function(itemNcb) {
      ...
      if (!item._id._pa.length) {
        ...
        if (length && DAGs[item._id._id][length - 1]._id._d) {
          ...
          that._log.info('_checkAncestry connected new root to previous (deleted) item', JSON.stringify(item));
        } else if (length) {
          ...
          that._log.err('_checkAncestry', error, JSON.stringify(item));
          throw error;
        }
      }
...
VersionedCollection.prototype._markConflicts = function _markConflicts(items, cb) {
  ...
  that._log.info('_markConflicts', JSON.stringify(items));
  ...
  that._log.debug('_markConflicts nonConflictedHeads', JSON.stringify(nonConflictedHeads));
  that._log.debug('_markConflicts conflictedHeads', JSON.stringify(conflictedHeads));
...
VersionedCollection.prototype._ensureOneHead = function _ensureOneHead(items, cb) {
  ...
  that._log.info('_ensureOneHead', JSON.stringify(items));

  var newHeads = VersionedCollection._branchHeads(items, true, false);

  that._log.info('_ensureOneHead new heads', JSON.stringify(newHeads));

  that._markConflicts(newHeads, function (err, nonConflictedNewHeads) {
    if (err) { that._log.err('err', err); cb(err); return; }
    if (nonConflictedNewHeads.length !== 1) {
      var error = new Error('not exactly one head');
      that._log.err('_ensureOneHead not exactly one head', error, nonConflictedNewHeads.length, JSON.stringify(items));
      cb(error);
      return;
    }
...
VersionedCollection.prototype._ensureVirtualCollection = function _ensureVirtualCollection(items, cb){
  ...
  that._log.info('_ensureVirtualCollection', JSON.stringify(items));

  try {
    var ac = new ArrayCollection(items, { debug: this.debug });
    this._virtualCollection = new ConcatMongoCollection([this._snapshotCollection, ac], { debug: this.debug });
    cb(null);
  } catch (err) {
    cb(err);
    return;
...
VersionedCollection.prototype._checkParentsInVirtualCollection = function _checkParentsInVirtualCollection(items, cb){
  ...
  async.eachSeries(items, function(item, cb2) {
    that._log.info('_checkParentsInVirtualCollection', JSON.stringify(item));
    ...
    async.each(item._id._pa, function(p, cb3) {
      ...
      that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_checkParentsInVirtualCollection' }, function(err, theParent) {
        if (err) {
          that._log.err('_checkParentsInVirtualCollection', err, JSON.stringify(item));
          return cb3(err);
        }
        if (!theParent) {
          var error = new Error('parent not found');
          that._log.err('_checkParentsInVirtualCollection', error, p, JSON.stringify(item), JSON.stringify(selector));
          return cb3(error);
        }
        cb3();
      });
...
VersionedCollection.prototype._mergeNewHeads = function _mergeNewHeads(items, newRoots, cb) {
  ...
  async.eachSeries(items, function(head, cb2) {
    that._log.info('_mergeNewHeads', JSON.stringify(head._id));
    ...
    that._snapshotCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_mergeNewHeads' }, function(err, localHead) {
      ...
      if (newRoots[head._id._id] && !head._id._pa.length && localHead && localHead._id._d) {
        ...
        that._log.info('_mergeNewHeads connected new root to previous (deleted) item', JSON.stringify(head));
      }
      ...
      if (newRoots[head._id._id] && !head._id._pa.length && localHead && !localHead._id._pa.length && localHead._id._v !== head._id._v) {
        ...
        that._log.err('_mergeNewHeads', head._id._id, error, JSON.stringify(head), JSON.stringify(localHead));
      ...
      if (!localHead) {
        that._log.info('_mergeNewHeads no local head found', JSON.stringify(head));
        return cb2();
      ...
      newThis._merge(localHead, head, function(err, merged) {
        ...
        if (err) {
          that._log.info('_mergeNewHeads merge error', head._id._id, err, JSON.stringify(localHead), JSON.stringify(head));
          // skip merge conflicts
          if (err.message === 'merge conflict') {
            // TODO: mark as conflict?
            that._log.notice('_mergeNewHeads merge conflict marked', head._id._id, err, JSON.stringify(localHead), JSON.stringify(head));
            head._m3._c = true;
            return cb2();
          }
          return cb2(err);
        ...
        if (merged[0]._id._v) {
          ...
          that._log.info('_mergeNewHeads ff without merge');
          return cb2();
        }

        that._log.info('_mergeNewHeads merge created', JSON.stringify(merged[0]._id));
...
VersionedCollection.prototype._ensureIntoSnapshot = function _ensureIntoSnapshot(perspective, localItems, remoteItems, cb) {
  ...
  try {
    ...
    that._log.debug('_ensureIntoSnapshot perspective', JSON.stringify(perspective));
    that._log.debug('_ensureIntoSnapshot remote items', JSON.stringify(remoteItems));
    that._log.debug('_ensureIntoSnapshot local items', JSON.stringify(localItems));
    ...
    that._log.debug('_ensureIntoSnapshot new local heads', JSON.stringify(newLocalHeads));
    that._log.debug('_ensureIntoSnapshot remote items length', remoteItems.length);
    that._log.debug('_ensureIntoSnapshot local items length', localItems.length);
    that._log.debug('_ensureIntoSnapshot new local heads length', newLocalHeads.length);

    // (re-)set sequence number on local items
    async.eachSeries(localItems, function(item, cb2) {
      that._getNextIncrement(function(err, i) {
        if (err) {
          that._log.err('_ensureIntoSnapshot add seq', err, i, item);
          return cb2(err);
        }
      ...
      async.eachSeries(allNew, function(newItem, cb3) {
        ...
        that._log.info('_ensureIntoSnapshot exists?', JSON.stringify(selector));

        that._snapshotCollection.findOne(selector, { comment: '_ensureIntoSnapshot' }, function(err, exists) {
          if (err) {
            that._log.err('_ensureIntoSnapshot CRITICAL', newItem._id, err);
            return cb3(err);
          }
          if (exists) { that._log.info('_ensureIntoSnapshot already exists', JSON.stringify(exists), JSON.stringify(newItem)); }

          if (!exists) {
            that._snapshotCollection.insert(newItem, {w: 1, comment: '_insertIntoSnapshot' }, function(err, inserted) {
              if (err) {
                that._log.err('_insertIntoSnapshot CRITICAL', err, JSON.stringify(newItem));
                return cb3(err);
              }
              that._log.info('_insertIntoSnapshot inserted', JSON.stringify(inserted));
              cb3();
            });
          } else {
            cb3();
...
VersionedCollection.prototype._insertIntoSnapshot = function _insertIntoSnapshot(perspective, localItems, remoteItems, cb) {
  ...
  try {
    ...
    that._log.info('_insertIntoSnapshot perspective', JSON.stringify(perspective));
    that._log.info('_insertIntoSnapshot remote items', JSON.stringify(remoteItems));
    that._log.info('_insertIntoSnapshot local items', JSON.stringify(localItems));
    ...
    that._log.info('_insertIntoSnapshot new local heads', JSON.stringify(newLocalHeads));
    that._log.info('_insertIntoSnapshot remote items length', remoteItems.length);
    that._log.info('_insertIntoSnapshot local items length', localItems.length);
    that._log.info('_insertIntoSnapshot new local heads length', newLocalHeads.length);

    // (re-)set sequence number on local items
    async.eachSeries(localItems, function(item, cb2) {
      that._getNextIncrement(function(err, i) {
        if (err) {
          that._log.err('_insertIntoSnapshot add seq', err, i, JSON.stringify(item));
          return cb2(err);
        }
      ...
      that._snapshotCollection.insert(allNew, {w: 1, comment: '_insertIntoSnapshot' }, function(err, inserted) {
        if (err) {
          that._log.err('_insertIntoSnapshot CRITICAL', err);
          return cb(err);
        }
        that._log.info('_insertIntoSnapshot inserted #', inserted);
        cb(null, newLocalHeads);
      });
    });
  } catch (err) {
...
VersionedCollection.prototype._syncLocalHeadsWithCollection = function _syncLocalHeadsWithCollection(newLocalHeads, cb) {
  ...
  that._log.info('_syncLocalHeadsWithCollection', JSON.stringify(newLocalHeads));

  async.eachSeries(newLocalHeads, function(head, cb2) {
    that._syncDAGItemWithCollection(head, function(err) {
      if (err) {
        that._log.err('_syncLocalHeadsWithCollection CRITICAL', err);
        return cb2(err);
      }

      that._log.info('_syncLocalHeadsWithCollection synced', JSON.stringify(head));
          ...
          that._ensureVirtualCollection(items, function(err) {
            ...
            that._checkParentsInVirtualCollection(items, function(err) {
              ...
              that._ensureLocalPerspective(items, function(err, newOrExistingLocalItems) {
                ...
                that._log.info('_ensureLocalPerspective new local items', JSON.stringify(newOrExistingLocalItems));

                if (newOrExistingLocalItems.length) {
                  if (perspective === that.localPerspective) {
                    var error = new Error('local duplicates created');
                    that._log.err('_ensureAllInDAG', error);
                    cb2(error);
                    return;
                  }
...
VersionedCollection.prototype.fixConsistency = function fixConsistency(pe, cb) {
  ...
  that._log.info('fixConsistency perspective', JSON.stringify(pe));
...
VersionedCollection.prototype._ensureLocalPerspective = function _ensureLocalPerspective(items, cb) {
  ...
  try {
    ...
    items.forEach(function(item) {
      if (perspective !== item._id._pe) {
        that._log.err('_ensureLocalPerspective perspective mismatch', perspective, item._id._pe);
        throw new TypeError('perspective mismatch');
      }
    });
  } catch(err) {
  ...
  async.eachSeries(items, function(item, cb1) {
    ...
    if (err) { cb(err); return; }
    var ac = new ArrayCollection(newItems, { debug: that.debug });
    that._virtualCollection = new ConcatMongoCollection([that._snapshotCollection, ac], { debug: that.debug });
    ...
    async.eachSeries(items, function(item, cb2) {
      that._log.info('_ensureLocalPerspective 1 new item', JSON.stringify(item._id));
      ...
      that._virtualCollection.findOne(selector, opts, function(err, lastLocalItem) {
        ...
        that._log.info('_ensureLocalPerspective 1 last local item', JSON.stringify(lastLocalItem));
        ...
        newThis._isAncestorOf(item._id._v, lastLocalItem, function(err, isAncestor) {
          ...
          if (isAncestor) {
            that._log.info('_ensureLocalPerspective 2 is ancestor of last local item', item._id._v);
          ...
          newThis._findLCAs(item, lastLocalItem, function(err, lcas) {
            ...
            if (lcas.length < 1) {
              ...
              that._log.err(item._id._id, '_ensureLocalPerspective 2', error, description);
            ...
            if (lcas.length > 1) {
              that._log.info('_ensureLocalPerspective 2 parent strategy', JSON.stringify(lcas[0]));

              if (item._id._pa.length > 2) {
                error = new Error('merging items with more than two parents and multiple lcas is not supported yet');
                that._log.err('_ensureLocalPerspective 2', error, description);
                cb2(error);
                return;
              }
              ...
              that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_ensureLocalPerspective_3' }, function(err, p1) {
                ...
                if (!p1) {
                  ...
                  that._log.err('_ensureLocalPerspective 3', error, description);
                  return cb2(error);
                }
                ...
                that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_ensureLocalPerspective_3' }, function(err, p2) {
                  ...
                  that._log.info('_ensureLocalPerspective 3 fetched p2', JSON.stringify(p2._id));

                  // create a perspective bound item by merging two parents
                  newThis._merge(p1, p2, function(err, merged) {
                    if (err) {
                      that._log.err('_ensureLocalPerspective 4 merge', err);
                      return cb2(err);
                    }

                    if (merged[0]._id._v) {
                      error = new Error('existing version used');
                      that._log.err('_ensureLocalPerspective 4', error, JSON.stringify(merged));
                      return cb2(error);
                    }
                    ...
                    that._log.info('_ensureLocalPerspective 4 merge created', JSON.stringify(merged[0]));
              ...
              that._log.info('_ensureLocalPerspective 2 lca strategy', JSON.stringify(lcas[0]));
              ...
              that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_ensureLocalPerspective_3' }, function(err, lca) {
                ...
                that._log.info('_ensureLocalPerspective 3 fetched lca', JSON.stringify(lca._id));
                ...
                newThis._merge(item, lca, function(err, merged) {
                  ...
                  if (merged[1]._m3) {
                    that._log.info('_ensureLocalPerspective 4 local perspective already exists', JSON.stringify(merged[1]));
                    cb2();
                    return;
                  }
                  ...
                  that._log.info('_ensureLocalPerspective 4 merge created', JSON.stringify(merged[1]));
      ...
      if (err) {
        that._log.err('_ensureLocalPerspective', err);
        return cb(err);
      }
...
VersionedCollection.prototype._merge = function _merge(objX, objY, cb) {
  this._log.info('_merge objX', JSON.stringify(objX._id), 'objY', JSON.stringify(objY._id));

  try {
    if (!VersionedCollection.equalValues(objX._id._id, objY._id._id)) { throw new Error('merge id mismatch'); }
  } catch(err) {
    this._log.err('_merge', err, JSON.stringify(objX._id._id), JSON.stringify(objY._id._id));
    process.nextTick(function() {
      cb(err, null);
    });
    return;
  ...
  function done(err, nlcas, perspectives) {
    ...
    if (err) {
      that._log.err('_merge err', err, nlcas, perspectives, debugObj);
      cb(err);
      return;
    }
    if (!nlcas.length) {
      that._log.err('_merge no lca found', debugObj);
      cb(new Error('no lca found'));
      return;
    }
    if (!perspectives.length) {
      that._log.err('_merge no perspectives', debugObj);
    ...
    if (effectiveNumberOfLCAs === 1) {
      ...
      if (perspectives.length === 1) {
        ...
        if (Array.isArray(merge)) {
          that._log.err('_merge error', merge, 'itemX', JSON.stringify(itemXid), 'itemY', JSON.stringify(itemYid));
      ...
      if (perspectives.length === 2) {
        ...
        if (lcaIds[0]._v && lcaIds[0]._v === itemXid._v) {
          ...
          if (Array.isArray(merged[0])) {
            that._log.err('_merge error', merged, 'itemX', JSON.stringify(itemXid), 'itemY', JSON.stringify(itemYid));
  ...
  that._findLCAs(itemX, itemY, function(err, lcas) {
    debugObj.lcas = lcas;
    if (err) {
      that._log.err('_merge error', err, debugObj);
      cb(err, debugObj);
      return;
    }
    if (lcas.length < 1) {
      that._log.err('_merge no lca found', JSON.stringify(debugObj), perspectives);
    ...
    that._snapshotCollection.find(selector, { comment: '_merge' }).toArray(function(err, lcaPerspectives) {
      ...
      if (lcaPerspectives.length !== lcas.length * perspectives.length) {
        that._log.err(that.databaseName, that.collectionName,
...
VersionedCollection.prototype._applyOplogInsertItem = function _applyOplogInsertItem(oplogItem, cb) {
  this._log.info('_applyOplogInsertItem', JSON.stringify(oplogItem));
  this._applyOplogUpdateFullDoc(oplogItem, cb);
};
...
VersionedCollection.prototype._applyOplogUpdateFullDoc = function _applyOplogUpdateFullDoc(oplogItem, cb) {
  this._log.info('_applyOplogUpdateFullDoc', JSON.stringify(oplogItem));
  ...
  that._log.info('_applyOplogUpdateFullDoc selector', JSON.stringify(selector));
  this._snapshotCollection.findOne(selector, { comment: '_applyOplogUpdateFullDoc' }, function(err, item) {
    ...
    that._log.info('_applyOplogUpdateFullDoc comparing', JSON.stringify(oplogItem.o), JSON.stringify(item));

    if (item && that.compareDAGItemWithCollectionItem(item, oplogItem.o)) {
      that._log.info('_applyOplogUpdateFullDoc, set ackd', JSON.stringify(item._id));

      // it should not be ackd
      if (item._m3._ack) { console.log('_applyOplogUpdateFullDoc item already ackd', JSON.stringify(item)); }

      // set ackd
      that._setAckd(item._id._id, item._id._v, item._id._pe, oplogItem.ts, function(err) {
        if (err) {
          that._log.err('_applyOplogUpdateFullDoc set ackd err', err);
          cb(err, oplogItem);
          return;
        }
      ...
      that._log.info('_applyOplogUpdateFullDoc create new version', JSON.stringify(oplogItem));
      ...
      that._findLastAckdOrLocallyCreated(oplogItem.o._id, function(err, item2) {
        ...
        if (oplogItem.op === 'u' && !item2) {
          ...
          that._log.err('_applyOplogUpdateFullDoc', error);
...
VersionedCollection.prototype._applyOplogUpdateModifier = function _applyOplogUpdateModifier(oplogItem, cb) {
  this._log.info('_applyOplogUpdateModifier', JSON.stringify(oplogItem));
...
VersionedCollection.prototype._applyOplogDeleteItem = function _applyOplogDeleteItem(oplogItem, cb) {
  this._log.info('_applyOplogDeleteItem', JSON.stringify(oplogItem));
...
VersionedCollection.prototype._findLastAckdOrLocallyCreated = function _findLastAckdOrLocallyCreated(id, cb) {
  ...
  this._log.info('_findLastAckdOrLocallyCreated selector', JSON.stringify(selector));

  var that = this;
  // selector and sort combination should use index created by _createSnapshotCollection
  this._snapshotCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_findLastAckdOrLocallyCreated' }, function(err, lastHead) {
    if (err) { return cb(err); }
    that._log.info('_findLastAckdOrLocallyCreated lastHead', JSON.stringify(lastHead));

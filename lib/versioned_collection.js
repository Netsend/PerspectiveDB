 ...
 *                                      queue.
 *   queueLimitRetryTimeout {Number, default 7000}  milliseconds between retrying a
...
function VersionedCollection(db, collectionName, options) {
  ...
  this._queueLimitRetryTimeout = options.queueLimitRetryTimeout || 7000; // timeout if queue is full in ms
...
VersionedCollection.prototype.saveRemoteItem = function saveRemoteItem(item, afterCb, cb) {
  ...
  if (!queue) {
    if (that.debug) { console.log(that.databaseName, that.collectionName, 'saveRemoteItem add new remote', item._id._pe); }
  ...
  function handleData(item) {
    ...
    async.eachSeries(item._id._pa, function(p, callback) {
      if (heads[p]) {
        ...
        walkBranch(lastEmitted, p, that.localPerspective, that._snapshotCollection, function(err, anItem, s) {
          if (err) {
...
VersionedCollection.prototype._createNewVersionByUpdateDoc = function _createNewVersionByUpdateDoc(dagItem, oplogItem, cb) {
  ...
  that._tmpCollection.insert(dagItem, {w: 1, comment: '_createNewVersionByUpdateDoc'}, function(err, inserted) {
    ...
    if (inserted.length !== 1) {
      ...
      if (!that._hide) { console.error(that.databaseName, that.collectionName, '_createNewVersionByUpdateDoc', error, JSON.stringify(dagItem), JSON.stringify(inserted)); }
      cb(error);
      return;
    }
    ...
    if (that.debug) { console.log(that.databaseName, that.collectionName, '_createNewVersionByUpdateDoc selector', JSON.stringify(selector)); }

    that._tmpCollection.findAndModify(selector, [], oplogItem.o, {w: 0, new: true}, function(err, newObj) {
      if (err) {
        if (!that._hide) { console.error(that.databaseName, that.collectionName, '_createNewVersionByUpdateDoc', err); }
        cb(err, oplogItem);
        return;
      }
      if (!newObj) {
        if (that.debug) { console.log(that.databaseName, that.collectionName, '_createNewVersionByUpdateDoc new doc not created', JSON.stringify(dagItem)); }
      ...
      that._tmpCollection.drop(function(err) {
        if (err) {
          if (!that._hide) { console.error(that.databaseName, that.collectionName, '_createNewVersionByUpdateDoc dropping _tmpCollection', err); }
...
VersionedCollection.prototype._isAncestorOf = function _isAncestorOf(version, item, cb) {
  ...
  walkBranch({ '_id._id': item._id._id, '_id._pe': item._id._pe }, item._id._v, this.localPerspective, this._snapshotCollection, function(err, anItem, stream) {
  ...
  try {
    ...
    items.forEach(function(itemNcb) {
      ...
      if (perspective !== item._id._pe) {
        if (!that._hide) { console.error(that.databaseName, that.collectionName, '_ensureSamePerspective perspective mismatch', perspective, item._id._pe); }
    ...
    async.eachSeries(localItems, function(item, cb2) {
      ...
      async.eachSeries(allNew, function(newItem, cb3) {
        ...
        that._snapshotCollection.findOne(selector, { comment: '_ensureIntoSnapshot' }, function(err, exists) {
          ...
          if (that.debug && exists) { console.log(that.databaseName, that.collectionName, '_ensureIntoSnapshot already exists', JSON.stringify(exists), JSON.stringify(newItem)); }

          if (!exists) {
            that._snapshotCollection.insert(newItem, {w: 1, comment: '_insertIntoSnapshot' }, function(err, inserted) {
              if (err) {
                if (!that._hide) { console.error(that.databaseName, that.collectionName, '_insertIntoSnapshot CRITICAL', err, JSON.stringify(newItem)); }
                return cb3(err);
              }
              if (that.debug) { console.log(that.databaseName, that.collectionName, '_insertIntoSnapshot inserted', JSON.stringify(inserted)); }
              cb3();
            });
          } else {
            cb3();
...
VersionedCollection.prototype._insertIntoSnapshot = function _insertIntoSnapshot(perspective, localItems, remoteItems, cb) {
  ...
  try {
    ...
    async.eachSeries(localItems, function(item, cb2) {
      that._getNextIncrement(function(err, i) {
        if (err) {
          if (!that._hide) { console.error(that.databaseName, that.collectionName, '_insertIntoSnapshot add seq', err, i, JSON.stringify(item)); }
          return cb2(err);
        }
...
VersionedCollection.prototype._addAllToDAG = function _addAllToDAG(newItems, cb) {
  ...
  async.eachSeries(Object.keys(DAGs), function(id, callback) {
    ...
    var ac = new ArrayCollection(items, { debug: that.debug });
      ...
      that._virtualCollection.findOne(selector, opts, function(err, lastLocalItem) {
        ...
        newThis._isAncestorOf(item._id._v, lastLocalItem, function(err, isAncestor) {
          ...
          newThis._findLCAs(item, lastLocalItem, function(err, lcas) {
            ...
            if (lcas.length > 1) {
              if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective 2 parent strategy', JSON.stringify(lcas[0])); }
              ...
              that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_ensureLocalPerspective_3' }, function(err, p1) {
                ...
                that._virtualCollection.findOne(selector, { sort: { '_id._i': -1 }, comment: '_ensureLocalPerspective_3' }, function(err, p2) {
                  ...
                  if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective 3 fetched p2', JSON.stringify(p2._id)); }
                  ...
                  newThis._merge(p1, p2, function(err, merged) {
                    ...
                    if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective 4 merge created', JSON.stringify(merged[0])); }
              ...
              if (that.debug) { console.log(that.databaseName, that.collectionName, '_ensureLocalPerspective 2 lca strategy', JSON.stringify(lcas[0])); }

 ...
 *
 * @param {Function} cb  The first parameter will contain either an Error object or
 *                       null. The second parameter is the max oplog pointer
 */
VersionedCollection.prototype.maxOplogPointer = function lastByPerspective(cb) {
  if (this.debug) { console.log(this.databaseName, this.collectionName, 'maxOplogPointer'); }

  if (typeof cb !== 'function') { throw new TypeError('cb must be a function'); }

  var selector = { '_id._op': { '$exists' : true} };

  this._snapshotCollection.findOne(selector, { sort: { '_id._op' : { $natural: -1 } }, comment: 'maxOplogPointer' }, cb);
};
  ...
  function done(err, nlcas, perspectives) {
    ...
    if (effectiveNumberOfLCAs === 1) {
      ...
      lcas.forEach(function(lca, i) {
        ...
        if (lca._m3) { delete lca._m3._ack; delete lca._m3._op; }
      });

      var itemXid = itemX._id;
      itemX._id = itemX._id._id;
      // make sure _ack is never set
      if (itemX._m3) { delete itemX._m3._ack; delete itemX._m3._op; }

      var itemYid = itemY._id;
      itemY._id = itemY._id._id;
      // make sure _ack is never set
      if (itemY._m3) { delete itemY._m3._ack; delete itemY._m3._op; }

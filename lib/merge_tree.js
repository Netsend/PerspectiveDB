...
MergeTree.prototype.createLocalWriteStream = function createLocalWriteStream() {
  ...
  function handleMerge(lhead, obj, cb) {
    ...
    that._log.debug('mt createLocalWriteStream ACK %j, local: %j', obj.n.h, lhead);

    // make sure this is a merge of the current local head
    if (!lhead && obj.l || lhead && !obj.l) {
      error = 'not a merge of the existing local head';
      that._log.debug('mt createLocalWriteStream error %s, local: %j', error, obj.l && obj.l.h || obj.l);
      cb(new Error(error));
      return;
    } else if (lhead != (obj.l && obj.l.h.v)) {
      error = 'current local head differs from local version of this merge';
      that._log.debug('mt createLocalWriteStream error %s, local: %j', error, obj.l && obj.l.h || obj.l);
      cb(new Error(error));
      return;
    }

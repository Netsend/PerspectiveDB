    ...
    write: function(item, encoding, cb) {
      ...
      function handleStageItem(err, exists) {
        ...
        if (exists && isEqual(exists.b, item.b)) {
          // move with ancestors to local, see mergeWithLocal
          that._log.debug('merge_tree createLocalWriteStream ACK %j', item.h);

          // ack, move everything up to this item from the stage to the local tree
          // mark every moved ancestor in conflict except this ack'd item
          stage.insertionOrderStream({ id: item.h.id, last: item.h.v }).pipe(new Writable({
            objectMode: true,
            write: function(stageItem, enc, next) {
              // set conflict on all previous unacknowledged heads
              if (stageItem.h.v !== item.h.v) {
                stageItem.h.c = true;
                that._log.debug('merge_tree createLocalWriteStream forced conflict %j', item.h);
              } else {
                // copy meta info of the acknowledged head to the item in stage
                if (item.m) {
                  stageItem.m = item.m;
                }
              }
          ...
          // determine parent by last non-conflicting head in local
          that._log.debug('merge_tree createLocalWriteStream NEW %j', item.h);

          var heads = [];
          local.createHeadReadStream({ id: item.h.id, skipConflicts: true }).on('readable', function() {
            var head = this.read();
            ...
            that._log.debug2('merge_tree createLocalWriteStream found heads %s', heads);
  ...
  setTimeout(function() {
    async.eachSeries(that._perspectives, function(pe, cb2) {
      if (that._updatedPerspectives[pe]) {
        // The merge read stream reads a snapshot from the moment it starts. By
        // the time it's done new remote items might have been written so clear
        // the updated flag before starting the merge.
        that._updatedPerspectives[pe] = false;
        that.mergeWithLocal(that._pe[pe], cb2);
      } else {
        cb2();
      }
    }, function(err) {
      ...
      write: function(sitem, enc, cb2) {
        that._stage.getByVersion(sitem.h.v, function(err, inStage) {
          ...
          that._transform(sitem, function(err, nitem) {
            ...
            that._log.debug2('merge_tree _copyMissingToStage %s', sitem.h.v);
...
MergeTree.prototype._createMergeStream = function _createMergeStream() {
  ...
  var transformer = new Transform({
    objectMode: true,
    transform: function(sitem, enc, cb) {
      if (!sitem.h.pe) {
        error = new Error('sitem misses perspective');
        that._log.err('_createMergeStream %s %j', error, sitem.h);
        cb(error);
        return;
      }

      if (sitem.h.i) {
        error = new Error('sitem has h.i');
        that._log.err('_createMergeStream %s %j', error, sitem.h);
        cb(error);
        return;
      }
      ...
      sitem = xtend(sitem);
      sitem.h = xtend(sitem.h);

      that._log.debug2('_createMergeStream %s', sitem.h.v);
      ...
      local.createHeadReadStream(headOpts).pipe(new Writable({
        ...
        if (lheads.length > 1) {
          ...
          that._log.err('_createMergeStream error %s %j', error, lheads);
          cb(error);
          return;
        }

        if (!lheads.length) {
          that._log.debug('_createMergeStream fast-forward %j', sitem.h);

          // new item not in local yet, fast-forward
          //that._mergeHandler(sitem, null, shnext);
          stage.write(sitem, function(err) {
            if (err) { cb(err); return; }
            cb(null, {
              n: sitem,
              o: null,
        ...
        that._log.debug('_createMergeStream merge %j with %j', sitem.h, lhead.h);

        // merge stage with local
        var sX = new ConcatReadStream([ stage.createReadStream({ id: sitem.h.id, reverse: true }), local.createReadStream({ id: lhead.h.id, reverse: true }) ]);
        ...
        merge(sX, sY, opts, function(err, smerge, lmerge) {
          if (err) {
            if (err.name !== 'MergeConflict') {
              that._log.err('_createMergeStream merge error %s %j %j', err, sitem, lhead, err.stack);
              cb(err);
              return;
            }
            // mark sitem conflict for later processing and proceed
            sitem.h.c = true;
            that._log.notice('_createMergeStream merge conflict %s %j %j', err.conflict, sitem, lhead);
            stage.write(sitem, function(err2) {
          ...
          if (!lmerge.h.v) {
            ...
            that._log.info('_createMergeStream merge %j', smerge.h);
            ...
            that._log.info('_createMergeStream fast-forward %j', smerge.h);
            if (smerge.h.i) {
              that._log.err('_createMergeStream already in stage %j', smerge.h);
              throw new Error('err');
            }
            stage.write(sitem, function(err) {
              if (err) { cb(err); return; }
            ...
            that._log.info('_createMergeStream already exists in local %j', lmerge.h);
            stage.write(sitem, cb);
          }
        });
      }).on('error', cb);
    }
...
MergeTree.prototype._mergeStageWithLocal = function _mergeStageWithLocal(cb) {
  ...
  stage.insertionOrderStream().pipe(new Writable({
    ...
    write: function(sitem, enc, snext) {
      that._log.debug2('_mergeStageWithLocal process %s', sitem.h.v);
      var headOpts = {
        id: sitem.h.id,
        skipConflicts: true
      };
      stage.getHeads(headOpts, function(shead, shnext) {
        if (shead.h.v !== sitem.h.v) {
          that._log.debug2('_mergeStageWithLocal skip head %s, not this item %s', shead.h.v, sitem.h.v);
          shnext();
          return;
        }

        that._log.debug2('_mergeStageWithLocal item %s is a head', sitem.h.v);

        // merge with the one non-conflicting head in local
        var lheads = [];
        local.createHeadReadStream(headOpts).on('readable', function() {
          var lhead = this.read();
          ...
          if (!lheads.length) {
            that._log.debug('_mergeStageWithLocal fast-forward %j', shead.h);

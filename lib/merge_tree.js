    ...
    transform(shead, function(err, nitem) {
      ...
      function mergeNewHeadWithStage(newHead) {
        ...
        stage.getHeads(headOpts, function(head, snext) {
          ...
          // if no previous head, insert merge in staging and call merge handler with the new merge and the local head as parent
          if (!heads.length) {
          ...
          merge(sX, sY, opts, function(err, merged) {
            // if conflict, write one item with conflict flag set, don't call merge handler
            if (err) {
            ...
            // if fast-forward, write one item and call merge handler with the new merged head and the previous head from stage
            if (merged.h.v === newHead.h.v) {
              stage.write(newHead, function(err) {
                if (err) {
      ...
      if (!dmerge) {
        // already exsists in local tree, merge with existing heads and save new merge for perspective update of stree in local tree
        shead.h.pe = stree.name;
        ...
        async.eachSeries(dheads, function(dhead, cb2) {
          ...
          merge(sX, sY, opts, function(err, smerge, dmerge) {
            ...
            if (!dmerge.h.v) {
              ...
              var vSize = smerge.h.pa[0].length * 6 / 8;
              if (vSize < 1) {
                cb2(new Error('used versions too small'));
                return;
              }
              var h = crypto.createHash('sha256');
              h.update(BSON.serialize(smerge), 'base64');
              // read first vSize bytes for version
              var version = h.digest().toString('base64', 0, vSize);
              dmerge.h.v = version;
              smerge.h.v = version;

...
function MergeTree(db, opts) {
  ...
  // setup auto-merging, fire and forget. see _scheduleMerge on how to stop this train
  if (opts.autoMergeInterval) {
    that._scheduleMerge(opts.autoMergeInterval, true);
  }
}
 ...
 */
MergeTree.prototype.createRemoteWriteStream = function createRemoteWriteStream(remote) {
 ...
 */
MergeTree.prototype._scheduleMerge = function _scheduleMerge(timeout, repeat, cb) {
  ...
  setTimeout(function() {
    async.eachSeries(that._perspectives, function(pe, cb2) {
      ...
      if (repeat && !that.stopAutoMerge) {
        that._scheduleMerge(timeout, repeat, cb);
      } else {
        cb();
      }
    });

...
/**
 * Merge items with local via stage.
 *
 * Merge every non-conflicting head in stage in insertion order with the one non-
 * conflicting head in local. On successful merge call back with the merged result
 * and the head from local. If there is a merge conflict call back with the head
 * from stage, the head from local and an array of conflicting key names.
 */
    ...
    transform: function(sitem, enc, cb) {
      ...
      local.createHeadReadStream(headOpts).pipe(new Writable({
        ...
        merge(sX, sY, opts, function(err, smerge, lmerge) {
          ...
          // if a fast-forward for stree, update last by perspective in local
          if (!lmerge.h.v) {
            // merge
            ...
            // version already exists in local tree (has both h.v and h.i)
            that._log.debug('_createMergeStream set last by %s = %s', sitem.h.pe, lmerge.h.v);
            local.write(sitem, cb);
          }
        });
      }).on('error', cb);
    }

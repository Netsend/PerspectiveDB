    ...
    write: function(item, encoding, cb) {
      ...
      function handleStageItem(err, exists) {
        ...
        if (exists && isEqual(exists.b, item.b)) {
          ...
          that._log.debug('mt createLocalWriteStream ACK %j', item.h);
          ...
          stage.insertionOrderStream({ id: item.h.id, last: item.h.v }).pipe(new Writable({
            ...
            write: function(stageItem, enc, next) {
              ...
              if (stageItem.h.v !== item.h.v) {
                stageItem.h.c = true;
                that._log.debug('mt createLocalWriteStream forced conflict %j', item.h);
              } else {
          ...
          that._log.debug('mt createLocalWriteStream NEW %j', item.h);
          ...
          local.createHeadReadStream({ id: item.h.id, skipConflicts: true }).on('readable', function() {
            ...
            that._log.debug2('mt createLocalWriteStream found heads %s', heads);
  ...
  var transformer = new Transform({
    ...
    transform: function(sitem, enc, cb) {
      if (!sitem.h.pe) {
        ...
        that._log.err('mt _createMergeStream %s %j', error, sitem.h);
        cb(error);
        return;
      }
      ...
      that._log.debug2('mt _createMergeStream %s', sitem.h.v);
      ...
      local.createHeadReadStream(headOpts).pipe(new Writable({
        ...
        if (!lheads.length) {
          that._log.debug('mt _createMergeStream fast-forward %j', sitem.h);
        ...
        that._log.debug('mt _createMergeStream merge %j with %j', sitem.h, lhead.h);
        ...
        merge(sX, sY, opts, function(err, smerge, lmerge) {
          if (err) {
            if (err.name !== 'MergeConflict') {
              that._log.err('mt _createMergeStream merge error %s %j %j', err, sitem, lhead, err.stack);
              cb(err);
              return;
            }
            // mark sitem conflict for later processing and proceed
            sitem.h.c = true;
            that._log.notice('mt _createMergeStream merge conflict %s %j %j', err.conflict, sitem, lhead);
          ...
          if (!lmerge.h.v) {
            ...
            that._log.info('mt _createMergeStream merge %j', smerge.h);
            ...
            that._log.info('mt _createMergeStream fast-forward %j', smerge.h);
            if (smerge.h.i) {
              that._log.err('mt _createMergeStream already in stage %j', smerge.h);
              throw new Error('err');
            }
            stage.write(sitem, function(err) {
              if (err) { cb(err); return; }
            ...
            that._log.debug('mt _createMergeStream set last by %s = %s', sitem.h.pe, lmerge.h.v);

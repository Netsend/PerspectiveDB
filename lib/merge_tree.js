...
function MergeTree(db, opts) {
  ...
  // create trees
  this._pe = {};
  this._tmpPe = {}; // used for temporarily saving merges (while async confirm is awaited)
...
/**
 * Remove a remote tree.
 *
 * @param {String} name of the remote
 * @param {Function} cb  first parameter will be an error or null
 */
MergeTree.prototype.rmRemote = function rmRemote(name, cb) {
  if (typeof name !== 'string') { throw new TypeError('name must be a string'); }
  if (typeof cb !== 'function') { throw new TypeError('cb must be a function'); }

  var tree = this._pe[name];
  if (!tree) {
    process.nextTick(function() {
      cb(new TypeError('remote tree not found'));
    });
    return;
  }
  ...
  var prefix = Tree.getPrefix(name);
  var end = Tree.getPrefix(name);
  end[end.length - 1] = 0x01; // change 0x00 to 0x01
    ...
    transform: function (item, enc, cb) {
      ...
      runHooks(hooks, that._db, item, hooksOpts, function(err, afterItem) {
        ...
        findConnectedParents(afterItem, filter, function(err, parents) {
          ...
          // don't remove meta info, i.e. mongo meta needs to be copied to others that use mongo
...
MergeTree.prototype.createLocalWriteStream = function createLocalWriteStream() {
  ...
  function handleMerge(lhead, obj, cb) {
    ...
    if (!lhead && obj.l || lhead && !obj.l) {
      // err, unless one of the parents directly references the head
    ...
    // expect all items to be in the tmp tree
    var items = [];

    // merge remote item with latest local head
    var found; // offset of this version
    var tmpTree = that._tmpPe[obj.pe];
    for (var i = tmpTree.length; i-- > 0;)
      if (tmpTree[i].n.h.id === obj.n.h.id)
        if (tmpTree[i].n.h.v === obj.n.h.v) {
          found = i;
          i = 0;
        }

    if (typeof found !== 'number') {
      cb(new Error('merge item not found in local tmp tree'));
      return;
    }

    // add everything of this id to the items that need to be saved
    // simultaneously remove from the tmp tree
    i = 0;
    while (i <= found) {
      if (tmpTree[i].n.h.id === obj.n.h.id) {
        items.push(tmpTree.splice(i, 1)[0].n);
      }
      i++;
    }

    that._log.debug('mt createLocalWriteStream copy %s items', items.length);
...
/**
 * Assume temp trees only contain one head per id.
 *
 * @param {String} id  id to search for
 * @param {String} remote  name of the temporary remote tree to include
 */
MergeTree.prototype._findTmpHead = function _findTmpHead(id, remote) {
  var tmpTree = this._tmpPe[remote];
  if (!tmpTree) { throw new Error('temp tree of remote not found'); }

  var head;
  for (var i = tmpTree.length; i-- > 0;) {
    if (tmpTree[i].n.h.id === id) {
      head = tmpTree[i];
      break;
    }
  }
...
/**
 * Find the latest local head in the local tree, optionally including any newly
 * created merges with a remote.
 *
 * @param {String} id  id to search for
 * @param {String} [remote]  name of the temporary remote tree to include
 * @param {Function} cb  First parameter will be an error or null, second parameter
 *   will be the local head or null if none found.
 */
MergeTree.prototype._findLatestHead = function _findLatestHead(id, remote, cb) {
  // first see if there is a newly created head
  var head;
  if (remote) {
    head = this._findTmpHead(id, remote);
    if (head) {
      process.nextTick(function() {
        cb(null, head.n);
      });
  ...
  // else check the local tree
  var heads = [];
  this._local.createHeadReadStream({ id: id }).on('data', function(lhead) {
    heads.push(lhead);
  }).on('end', function() {
    if (heads.length > 1) {
      var error = new Error('more than one head in local');
      that._log.err('mt _findLocalHead error %s %j', error, heads);
      cb(error);
      return;
    }
    cb(null, heads[0]);
...
/**
 * Merge items between a remote and the local tree.
 *
...
MergeTree.prototype._createMergeStream = function _createMergeStream() {
  ...
  var transformer = new Transform({
    ...
    transform: function(ritem, enc, cb) {
      ...
      var rtmpTree = that._tmpPe[ritem.h.pe];
      if (!rtree) {
      ...
      // merge with the latest local head, including any unconfirmed previous merges with this remote
      that._findLatestHead(ritem.h.id, ritem.h.pe, function(err, lhead) {
        ...
        // use remote tmp array
        var streamifyOpts = {
          reverse: true,
          filter: function(el) {
            return el.n.h.id === ritem.h.id;
          },
          map: function(el) {
            return el.n;
          }
        ...
        var sY = new ConcatReadStream([streamify(rtmpTree, streamifyOpts), local.createReadStream({ id: lhead.h.id, reverse: true })]);
        ...
        merge(sX, sY, function(err, rmerge, lmerge, lcas) {
          ...
          if (!lmerge.h.v) {
            ...
            rtmpTree.push({ n: ritem }); // make sure ritem is in the array as well
            rtmpTree.push(item);
            cb(null, item);
          } else if (!lmerge.h.i) {

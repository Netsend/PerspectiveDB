 ...
 *   autoMergeInterval {Number, default 0}  milliseconds between merging new items
 *                                          from different perspectives to the
 *                                          local tree.
...
function MergeTree(db, opts) {
  ...
  if (opts.autoMergeInterval != null && typeof opts.autoMergeInterval !== 'number') { throw new TypeError('opts.autoMergeInterval must be a number'); }
  ...
  if (opts.mergeHandler) {
    ...
    // immediately confirm writes if no mergehandler is provided
  ...
  // setup auto-merging, fire and forget. see scheduleMerge on how to stop this train
  if (opts.autoMergeInterval) {
    that.scheduleMerge(opts.autoMergeInterval, true);
  }
...
MergeTree.prototype.createRemoteWriteStream = function createRemoteWriteStream(remote, opts) {
  ...
  if (!tree) {
    ...
    that._log.err('merge_tree %s %s', error, remote);
    throw new Error(error);
  }

  this._log.info('merge_tree opened tree %s', remote);
  ...
  return through2.obj(function(item, encoding, cb) {
    try {
      ...
      that._log.err('merge_tree %s %s', err, remote);
...
/**
 * Schedule a merge of every updated perspective with local. If repeat is given a
 * loop is started that can be interrupted by setting mergeTree.stopAutoMerge to
 * true. The call back is called only once.
 *
 * @param {Number} timeout  time in ms to postpone execution
 * @param {Boolean} [repeat]  if a new merge has to be scheduled after completion
 * @param {Function} [cb]  Called after merge is executed. First parameter will be
 *                         an error object or null.
 */
MergeTree.prototype.scheduleMerge = function scheduleMerge(timeout, repeat, cb) {
  ...
  if (repeat != null && typeof repeat !== 'boolean') { throw new TypeError('repeat must be a boolean'); }
  ...
  setTimeout(function() {
    async.eachSeries(that._perspectives, function(pe, cb2) {
      ...
      //that._log.debug('mt scheduleMerge done');

      if (repeat && !that.stopAutoMerge) {
        that.scheduleMerge(timeout, repeat, cb);
      } else {
        cb();
      }
    });
...
MergeTree.prototype.close = function close(cb) {
  ...
  setTimeout(cb, (this._opts.autoMergeInterval || 0) + 100); // wait at least 100ms
};

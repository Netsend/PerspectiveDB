 ...
 *
 * It is assumed that each DAG in the local tree has exactly one head.
 *
...
MergeTree.prototype.mergeWithLocal = function mergeWithLocal(stree, cb) {
  ...
  function iter(sitem, lhead, next) {
    var merge = that._merge(sitem, lhead);
    if (Array.isArray(merge)) {
      // merge conflict, do nothing
      that._log.warn('mergeStageWithLocal merge conflict %s %j %j', merge, sitem, lhead);
      next();
      return;
    }
    ...
    stage.write(merge[1], function() {
      that._mergeHandler(merge[1], lhead, next);
    });
  }

  this._iterateMissing(stree, this._local, iter, cb);
};
...
/**
 * Iterate over item in src starting at the last offset of stree in dtree.
 *
 * Ensures insertion order of src.
 *
 * @param {Object} stree  source tree
 * @param {Object} dtree  destination tree
 * @param {Function} iterator  function(sitem, dhead, next) called with src
 *                             head, dst head and next handler
 * @param {Function} cb  First parameter will be an error object or null.
 */
MergeTree.prototype._iterateMissing = function _iterateMissing(stree, dtree, iterator, cb) {
  if (!t.isObject(stree)) { throw new TypeError('stree must be an object'); }
  ...
  // determine offset
  dtree.lastByPerspective(stree.name, function(err, v) {
    if (err) { cb(err); return; }
    ...
    stree.iterateInsertionOrder(opts, function(sitem, snext) {
      // get all corresponding heads in dtree, iterate with each one of them

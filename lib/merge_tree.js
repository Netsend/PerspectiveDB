  ...
  return through2.obj(function(item, encoding, cb) {
    ...
    stage.getByVersion(item.h.v, function(err, exists) {
      ...
      if (exists && isEqual(exists.b, item.b)) {
        // use item from stage and copy any meta info
        exists.m = item.m;
        item = exists;
        ...
        // add new version, determine parent by last non-deleted and non-conflicting head (should always be either none or one item in the local tree)
        if (item.h.pa) {
        ...
        local.getHeads({ id: item.h.id, skipDeletes: true, skipConflicts: true }, function(head, next) {
          ...
          item.h.v = MergeTree.versionContent(item);

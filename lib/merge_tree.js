 ...
 * every head that is not in the local tree with the head in the local tree. Call
 * merge handler with the new head and the current head in local.
 ...
 *                                      signature: function (err, shead, lhead, cb)
 *                                      call cb with a new item or null if not
 *                                      resolved.
 */
MergeTree.prototype._ensureConflictHandler = function _ensureConflictHandler(conflictHandler) {
  ...
  if (!this._conflictHandler) {
    this._conflictHandler = function(err, shead, lhead, cb) {
      cb();
    };
  }
};
...
/**
 * Merge src tree with dst tree. Find the last version of src tree that is in dst
 * tree and try to merge each head in src (in insertion order) with every head of
 * the DAG in dst tree. Call back with the head from src, the head from dst and the
 * merged result. If there is a merge conflict, smerge in the iterator will be an
 * array of conflicting attributes and no dmerge is set. If the merge is a fast-
 * forward for one of the trees, either smerge or dmerge is passed back, not both.
 * shead and dhead are always passed back, smerge and dmerge only if new.
 *
 * Iterator is called (heads in stree) * (heads in dtree) times.
 *
 * @param {Object} stree  source tree to merge
        ...
        async.eachSeries(dheads, function(dhead, cb2) {
          ...
          merge(sX, sY, opts, function(err, smerge, dmerge) {
            ...
            if (!smerge.h.v) {
              ...
              var version = MergeTree._versionContent(smerge); // merkle-tree
              smerge.h.v = version;
              dmerge.h.v = version;
              ...
              // existing item (has both h.v and h.i)

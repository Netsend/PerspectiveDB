...
MergeTree.prototype.mergeWithLocal = function mergeWithLocal(stree, cb) {
  ...
  function iterator(merge, sitem, ditem, next) {
    ...
    if (lastId !== ditem.h.id) {
      ...
      lastId = ditem.h.id;
      prev = [];
      that._log.err('mergeWithLocal more than one local head %j %j', lhead);
 ...
 * non-deleted head of the DAG in dst tree. Call back with the result.
 *
 * @param {Object} stree  source tree to merge
 * @param {Object} dtree  dest tree to merge with
 * @param {Function} iterator  function(smerge, smerge, spa, dpa, next) called with
 *                             merges from both perspectives and both parents.
...
MergeTree._mergeTrees = function _mergeTrees(stree, dtree, iterator, cb) {
  ...
  MergeTree._iterateMissing(stree, dtree, function(sitem, snext) {
    ...
    stree.getHeads({ id: sitem.h.id }, function(shead, shnext) {
      ...
      // merge with all non-deleted head(s) in dtree
      var dheads = [];
      dtree.getHeads(headOpts, function(dhead, dnext) {
        ...
        if (!dheads.length) {
          ...
          iterator(shead, null, shead, null, shnext);
          return;
        }

        // merge with all dheads
        async.eachSeries(dheads, function(dhead, cb2) {
          merge(shead, dhead, stree, dtree, function(err, smerge, dmerge) {

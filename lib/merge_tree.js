...
MergeTree.prototype.mergeWithLocal = function mergeWithLocal(stree, cb) {
  ...
  var headOpts = { skipConflicts: true };

  function iterator(sitem, snext) {
    ...
    that._local.getHeads(headOpts, function(dhead, dnext) {
      if (lhead) {
        ...
        dnext(new Error('multiple heads in local DAG encounterd'));
        return;
      }
      if (!dhead.h.d) {
        lhead = dhead;
      ...
      if (Array.isArray(merge)) {
        ...
        that._log.warn('mergeWithLocal merge conflict %s %j %j', merge, sitem, lhead);
        snext();
        return;
      }

      // insert merge in staging and call mergeHandler with the new merge and the old local perspective
      stage.write(merge[1], function(err) {
        if (err) {
          // write error in stage
          that._log.err('mergeWithLocal stage write error %j %j %j %s', sitem, lhead, merge, err);
          snext(err);
          return;
        }
        that._mergeHandler(merge[1], lhead, snext);
      });
    });
  }

  MergeTree._iterateMissing(stree, this._local, iterator, cb);
};

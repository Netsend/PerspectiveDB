 ...
 * ca = []
 * for each v in V
 *   if v in next // then on path
 *     newDepth = next[v] + 1
 *     for each p in parents(v)
 *       next[p] = newDepth
 *     delete next[v]
 *     if depth < newDepth
 *       slca = finishDepth(depth)
 *       if (slca)
 *         return slca
 *       depth = newDepth
 *     // depth == newDepth
 *     for each level in ancestorsOther
 *       if v in level
 *         ca[depth][v] = true
 *       levelca = []
 ...
 *           levelca <- p
 *       ca[depth + 1] = levelca
 *     ancestorsSame[depth] = addParentGroup parents(v)
 *
 * finishDepth(level)
 *   if ca[level].length == 1, then single lca
 *     return ca[level][0]
 *   else
 *     return undefined
 *

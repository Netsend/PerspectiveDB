...
function VersionedCollectionReader(snapshotCollection, opts) {
  ...
  this._debugName = this._db.databaseName + '.' + this._snapshotCollectionName;
  ...
  if (this._debug) { console.log('vcr %s: selector %s filter %s opts %s', this._debugName, JSON.stringify(selector), JSON.stringify(this._filter), JSON.stringify(mongoOpts)); }
  ...
  function handleData(item) {
    if (that._debug && offsetReached) { console.log('vcr %s: _read %s', that._debugName, JSON.stringify(item._id)); }

    // only start emitting after the offset is encountered
    if (!offsetReached) {
      if (item._id._v === that._offset) {
        if (that._debug) { console.log('vcr %s: _read offset reached %s', that._debugName, JSON.stringify(item._id)); }
        offsetReached = true;
      } else {
        // the offset should be encountered within maxTries
        i++;
        if (i >= that._maxTries) {
          if (!that._hide) { console.error('vcr %s: _read offset not found %s %s %s', that._debugName, that._offset, i, that._maxTries); }
    ...
    async.eachSeries(item._id._pa, function(p, callback) {
      if (heads[p]) {
        ...
        walkBranch(lastEmitted, p, that._localPerspective, that._snapshotCollection, function(anItem, cb) {
          ...
          if (err) {
            if (!that._hide) { console.error('vcr %s %s could not determine last emitted version before %s %s %s', p, lastEmitted, err); }
            callback(err);
            return;
          }
          callback();
      ...
      runHooks(that._hooks, that._db, item, that._hooksOpts, function(err, afterItem) {
        ...
        if (!afterItem) {
          if (that._debug && offsetReached) { console.log('vcr %s: vcr hook filtered %s', that._debugName, JSON.stringify(item._id)); }
          that._source.resume();
          return;
        }
        ...
        if (offsetReached) {
          ...
          if (that._debug) { console.log('vcr %s: push %s', that._debugName, JSON.stringify(item._id)); }
  ...
  this._source.on('error', function(err) {
    if (!that._hide) { console.error('vcr %s: cursor stream error %s', that._debugName, err); }
    that.emit('error', err);
  });

  this._source.on('close', function() {
    if (that._debug) { console.log('vcr %s: cursor stream closed', that._debugName); }
    that.push(null);
  });
}
...
VersionedCollectionReader.prototype.close = function close() {
  if (this._debug) { console.log('vcr %s: closing cursor', this._debugName); }

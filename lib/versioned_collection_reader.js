...
function VersionedCollectionReader(db, collectionName, opts) {
  ...
  if (this._debug) { console.log('vcr %s %s selector %s filter %s', this.databaseName, this.collectionName, JSON.stringify(selector), JSON.stringify(this.filter)); }
  ...
  function handleData(item) {
    if (that._debug && offsetReached) { console.log('vcr %s %s _read %s', that.databaseName, that.collectionName, JSON.stringify(item._id)); }

    // only start emitting after the offset is encountered
    if (!offsetReached) {
      if (item._id._v === that.offset) {
        if (that._debug) { console.log('vcr %s %s _read offset reached %s', that.databaseName, that.collectionName, JSON.stringify(item._id)); }
        offsetReached = true;
      } else {
        // the offset should be encountered within maxTries
        i++;
        if (i >= that.maxTries) {
          if (!that._hide) { console.error('vcr %s %s _read offset not found %s %s %s', that.databaseName, that.collectionName, that.offset, i, that.maxTries); }
    ...
    async.eachSeries(item._id._pa, function(p, callback) {
      if (heads[p]) {
        ...
        walkBranch(lastEmitted, p, that.localPerspective, that._snapshotCollection, function(err, anItem, s) {
          if (err) {
            if (!that._hide) { console.error('vcr %s %s could not determine last emitted version before %s %s %s %s', p, lastEmitted, JSON.stringify(err), JSON.stringify(anItem)); }
            return callback(err);
          }
          // when done
          if (!anItem) {
      ...
      VersionedCollectionReader.runHooks(that.hooks, that._db, item, that.hooksOpts, function(err, afterItem) {
        if (err) { return that._source.emit('error', err); }

        // if hooks filter out the item, do not callback since that would signal the end
        if (!afterItem) {
          if (that._debug && offsetReached) { console.log('vcr %s %s vcr hook filtered %s', that.databaseName, that.collectionName, JSON.stringify(item._id)); }
          that._source.resume();
          return;
        }
        ...
        if (offsetReached) {
          ...
          if (that._debug) { console.log('vcr %s %s push %s', that.databaseName, that.collectionName, JSON.stringify(item._id)); }
  ...
  this._source.on('error', function(err) {
    if (!that._hide) { console.error('vcr %s %s cursor stream error %s', that.databaseName, that.collectionName, JSON.stringify(err)); }
    that.emit('error', err);
  });

  this._source.on('close', function() {
    if (that._debug) { console.log('vcr %s %s cursor stream closed', that.databaseName, that.collectionName); }
    that.closed = true;
    that.push(null);
  });
}
...
VersionedCollectionReader.prototype.close = function close() {
  if (this._debug) { console.log('vcr %s %s closing cursor', this.databaseName, this.collectionName); }

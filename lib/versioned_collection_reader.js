...
function VersionedCollectionReader(db, collectionName, opts) {
  ...
  if (this._debug) { console.log('vcr %s %s selector %s filter %s', this.databaseName, this.collectionName, selector, this.filter); }
    ...
    async.eachSeries(item._id._pa, function(p, callback) {
      if (heads[p]) {
        ...
        that._walkBranch(lastEmitted, p, function(err, anItem, s) {
          if (err) {
            if (!that._hide) { console.error('vcr %s %s could not determine last emitted version before %s %s %s %s', p, lastEmitted, err, anItem); }
            return callback(err);
          }
          // when done
          if (!anItem) {

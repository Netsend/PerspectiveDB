...
function registerVcr(vcr, conn) {
  ...
  console.log('%s: vcr registering %s', programName, dest);

  vcrs.push(vcr);

  vcr.on('error', function(err) {
    console.error('%s: vcr error: %s %s', programName, err, dest);
  });

  vcr.once('end', function() {
    console.log('%s: vcr end: %s', programName, dest);
    ...
    vc.lastByPerspective(database, function(err, last) {
      ...
      function connectHandler(err, conn2) {
        ...
        rt.on('error', function(err) {
          console.error('remote transform error', err, JSON.stringify(conn2.address()));
        });
        ...
        conn2.once('end', function() {
          conn2.unpipe(bs2);
          console.log('%s: conn2 end unpiped conn2--bs2', programName);
          bs2.end();
        });

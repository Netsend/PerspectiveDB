...
function registerConnection(conn) {
  ...
  conn.on('error', function(err) {
    console.error('%s: conn error: %s %s', programName, err, connId);
  });
...
function startVc(db, cfg) {
  ...
  function handleIncomingMsg(req, conn) {
    ...
    if (pullRequest.valid(req)) {
      ...
      // convert all hooks to the loaded module
      ...
      var snapshotCollection = db.collection(cfg.collectionName);
      var vcr = new VersionedCollectionReader(snapshotCollection, req);

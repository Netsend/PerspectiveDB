...
function startVc(db, cfg) {
  ...
  process.stdin.pipe(bs).on('data', function(obj) {
    ...
    if (vc._queueLimit <= vc._oplogBuffer.length) {
      if (!vc._hide) { console.log('%s: %s.%s saveOplogItem queue full, wait and retry. %s %s', programName, vc.databaseName, vc.collectionName, vc._queueLimitRetryTimeout, vc._oplogBuffer.length); }

      bs.pause();

      setTimeout(function() {
        if (vc.debug) { console.log('%s: %s.%s saveOplogItem retry %s %s', programName, vc.databaseName, vc.collectionName, vc._queueLimitRetryTimeout, vc._oplogBuffer.length); }

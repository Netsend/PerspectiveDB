...
function startVc(db, cfg) {
      ...
      function connHandler(conn) {
        ...
        closePull.push(function(cb) {
          ...
          conn.setTimeout(200, function() {
            console.log('pull socket destroyed', conn.address());
            conn.destroy();
            cb();
          });
          conn.end();
    ...
    if (pullRequest.valid(req)) {
      ...
      closePush.push(function(cb) {
        vcr.once('end', function() {
          ...
          // give the peer 200ms to end
          conn.setTimeout(200, function() {
            console.log('push socket destroyed', conn.address());
            conn.destroy();
            cb();
          });
          conn.end();
  ...
  function shutdown() {
    ...
    async.series(closePull, function(err) {
      if (err) { throw err; }
      console.log('pull requests closed');
      async.series(closePush, function(err) {
        if (err) { throw err; }
        console.log('push requests closed');

        process.stdin.unpipe(bs);
        console.log('stdin oplog reader unpiped');
        ...
        bs.end(function(err) {
          if (err) { throw err; }
          console.log('bson stream ended');

          vc.stopAutoProcessing(function(err) {
            if (err) { throw err; }
            console.log('vc stopped processing');

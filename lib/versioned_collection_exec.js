 ...
 *
 * Full FSM: log1 --> log2 --> init --> listen
 *
 * The first message emitted is "log1" which signals that it's ready to receive
 * the first message that must contain the log configuration object and an optional
 * writable file stream object used for the "file" parameter in the log config.
 *
 * {
 *   logCfg:          {Object}      // log configuration object, process.stdout
 *                                  // will be mapped to file and process.stderr
 *                                  // wil be mapped to error
 *   [logFile]:       {Object}      // writable stream used as the file parameter
 * }
 *
 * The second message emitted is "log2" which signals that it's ready to receive
 * the second message that must contain the string "errFile" and an optional
 * writable file stream object used for the "error" parameter in the log config.
 *
 * {
 *   name:            {String}      // must be "errFile"
 *   [errFile]:       {Object}      // writable stream used as the error parameter
 * }
 *
 * The third message emitted is "init" which signals that it's ready to receive the
 * last piece of configuration data, namely the database and Versioned Collection
 * config objects.
 *
 * {
 *   dbName:          {String}
...
function startVc(db, cfg) {
  ...
  try {
    ...
    log.err('changing root or user failed %j', cfg, err);
    process.exit(1);
  }
    ...
    vc.lastByPerspective(database, function(err, last) {
      ...
      function connectHandler(err, conn2) {
        ...
        rt.on('error', function(err) {
          log.err('remote transform error %j %j', err, conn2.address());
        });
...
// expect one log configuration object and an optional file descriptor for the file option
process.once('message', function(logCfg, logFile) {
  if (typeof logCfg !== 'object') { throw new TypeError('logCfg must be an object'); }
  ...
  // expect second message with an optional file descriptor for the extra error log
  process.once('message', function(name, errFile) {
    if (name !== 'errFile') { throw new TypeError('expected name to be "errFile"'); }
    if (typeof errFile !== 'undefined' && typeof errFile !== 'object') { throw new TypeError('errFile must be an object'); }

    if (errFile) {
      logCfg.error = errFile;
    }

    process.send('init');

    // next, expect an init request (request that contains db name and collection name and any other opts)

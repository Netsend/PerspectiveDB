 ...
 * and setup Versioned Collection Readers for incoming push requests.
 *
 * 1. setup push requests to remotes
 * 2. handle incoming push request
 ...
 *
 *
 * Any subsequent messages sent to this process should be push requests accompanied
 * with a socket. The push request should have the following structure:
 *
 * {
 *   [filter]:     {}
 ...
 * 3. handle oplog items on stdin
 */
function startVc(db, chrootCfg, vcCfg, remotes) {
  ...
  // setup data requests to remotes
  async.eachSeries(remotes || [], function(remote, cb) {
    ...
    vc.lastByPerspective(remote, function(err, last) {
      ...
      function connHandler(conn) {
        ...
        var authReq = {
          ...
          database: remote.database || db.databaseName,
      ...
      if (remote.path) {
        ...
        net.connect(remote.port || 2344, remote.host || '127.0.0.1', connHandler);
      }
    });
  }, function(err) {
  ...
  // handle incoming push requests
  process.on('message', function(msg, conn) {
    console.log('%s: incoming ipc message', programName, msg);

    if (!pushRequest.valid(msg)) {
      connErrorHandler(conn, 'invalid push request');
      return;
    }
...
process.once('message', function(msg) {
  ...
  if (typeof msg.remotes !== 'undefined') {
    if (!Array.isArray(msg.remotes)) { throw new TypeError('msg.remotes must be an array'); }
  }

  // open database
  _db(msg.dbConfig, function(err, db) {
    if (err) { throw err; }
    startVc(db, msg.chrootConfig, msg.vcConfig, msg.remotes);
  });
});

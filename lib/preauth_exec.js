...
function connHandler(conn) {
  ...
  ls.once('end', function() {
    ...
    console.log('%s: ls end unpipe conn--ls', programName);
    conn.unpipe(ls);
    conn.end();
  ...
  conn.once('error', function(err) {
    console.log('%s: conn error unpipe conn--ls %s', programName, err);
    conn.unpipe(ls);
    ls.end();
  });

  conn.once('end', function() {
    console.log('%s: conn end', programName);
  ...
  if (host) {
    ...
    shutdownTasks.push(function(cb) {
      ...
      serverInet.getConnections(function(err, count) {
        ...
        setTimeout(function() {
          if (serverInetClosed) {
            console.log('%s: server closed', programName);
            return;
          }
          if (!count) {
            console.log('%s: server not closed, force quit', programName);
            process.exit();
          }
        }, 100);

  ...
  logger(msg.log, function(err, l) {
    ...
    if (ws) {
      ...
      wsChild.stdout.on('data', function(data) {
        log.debug('ws child stdout: "\n%s"', data);
      });
      wsChild.stderr.on('data', function(data) {
        log.err('ws child stderr: "\n%s"', data);
      });
    }
      ...
      if (wsChild) {
        startupTasks.push(function(cb) {
          ...
          wsChild.once('message', function(cmsg) {
            ...
            // send config
            wsChild.send({
              log:            msg.log,     // log configuration
              cert:           msg.cert,    // X.509 certificate for the TLS server
              key:            msg.key,     // RSA private key for the certificate
              dhparam:        msg.dhparam, // file that contains a Diffie-Hellman group

...
process.once('message', function(msg) {
  ...
  logger(msg.log, function(err, l) {
    ...
    if (ws) {
      ...
      log.info('wsChild spawned');

      wsChild.on('exit', function(code, signal) {
        log.info('wsChild exit', code, signal);
      });
      wsChild.on('close', function(code, signal) {
        log.info('wsChild close', code, signal);
      });
      wsChild.on('error', function(err) {
        log.err('wsChild error', err);
      });

      wsChild.stdout.setEncoding('utf8');
      wsChild.stderr.setEncoding('utf8');

      wsChild.stdout.on('data', function(data) {
        log.debug('wsChild stdout:', data);
      });
      wsChild.stderr.on('data', function(data) {
        log.err('wsChild stderr:', data);
      });
    }
    ...
    function openServerAndProceed() {
      ...
      startupTasks.push(function(cb) {
        tcpServer.listen(port, host, function(err) {
          if (err) {
      ...
      if (wsChild) {
        ...
        shutdownTasks.push(function(cb) {
          ...
          wsChild.on('close', function(code, signal) {
            if (code) { log.err('wsChild exit code: %d, signal: %s', code, signal); }
            cb();
          });
          wsChild.kill();
        });
      ...
      shutdownTasks.push(function(cb) {
        ...
        tcpServer.getConnections(function(err, count) {
          ...
          // TODO: check why tcpServer.close does not always emit close event, even when connection count is 0

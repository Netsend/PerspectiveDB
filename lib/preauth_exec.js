...
function connHandler(conn) {
  ...
  ls.on('error', function(err) {
    log.err('ldjsonstream error %s %s', connId, err);
    conn.unpipe(ls);
    conn.end();
    ls.end();
  });
...
process.once('message', function(msg) {
  ...
  var host;
  if (msg.serverConfig.hasOwnProperty('host')) {
    if (typeof msg.serverConfig.host !== 'string') { throw new TypeError('msg.serverConfig.host must be a number'); }
    host = msg.serverConfig.host;
  }

  var port = 2344;
  if (msg.serverConfig.hasOwnProperty('port')) {
    if (typeof msg.serverConfig.port !== 'number') { throw new TypeError('msg.serverConfig.port must be a number'); }
    port = msg.serverConfig.port;
  }

  // server config
  var path = '/var/run/ms-' + port + '.sock';
  if (msg.serverConfig.hasOwnProperty('path')) {
    if (typeof msg.serverConfig.path !== 'string') { throw new TypeError('msg.serverConfig.path must be a number'); }
    path = msg.serverConfig.path;
  }
  ...
  logger(msg.logCfg, function(err, l) {
    ...
    startupTasks.push(function(cb) { cb(); });
    ...
    if (host) {
      ...
      // open INET socket
      startupTasks.push(function(done) {

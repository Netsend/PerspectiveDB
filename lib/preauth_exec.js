 ...
 *
 * This module should be forked and exactly one message should be sent with the
 * following structure:
 *
 * {
 *   [serverConfig]: {}
...
function connHandler(conn) {
  ...
  conn.pipe(split(JSON.parse, null, { maxLength: 1024 })).once('data', function(req) {
...
process.once('message', function(msg) {
  ...
  var path = msg.serverConfig.path || '/tmp/preauth-' + port + '.sock';
  ...
  // cleanup any previously created sockets

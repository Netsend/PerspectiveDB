 ...
 *   hostHash:       {String}      // sha256 hash of the hosts fingerprint
...
process.once('message', function(msg) {
  ...
  if (!msg.hostHash || typeof msg.hostHash !== 'string') { throw new TypeError('msg.hostHash must be a string'); }
    ...
    dns.lookup(msg.host, function(err, hostIp) {
      ...
      tunnel.connect({
        ...
        hostVerifier: function(fingerprint) {
          if (fingerprint === msg.hostHash) {
            log.info('tunnel host key verified: %s', msg.hostHash);
            return true;
          } else {
            log.err('tunnel host key mismatch! expected %s got %s', msg.hostHash, fingerprint);
            return false;
          }
        }
      });

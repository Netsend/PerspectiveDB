...
process.once('message', function(msg) {
  ...
  logger(msg.log, function(err, l) {
    ...
    dns.lookup(msg.host, function(err, hostIp) {
      ...
      // ensure hex fingerprint
      var expectedFingerprint;
      if (msg.fingerprint.length === 64) {
        // run through buffer to ensure this is really a valid hex string
        expectedFingerprint = new Buffer(msg.fingerprint, 'hex').toString('hex');
      } else {
        expectedFingerprint = new Buffer(msg.fingerprint, 'base64').toString('hex');
      }
      ...
      tunnel.connect({
        ...
        hostHash: 'sha256',
        debug: log.debug2,
        hostVerifier: function(hostFingerprint) {
          if (hostFingerprint === expectedFingerprint) {
            log.info('tunnel host key verified: %s', msg.fingerprint);
            return true;
          } else {
            log.err('tunnel host key mismatch! expected %s got %s', expectedFingerprint, hostFingerprint);
            return false;
          }
        }
      });
    ...
    try {
      ...
      log.info('tunnel changed root to %s and user:group to %s:%s', newRoot, user, group);
    } catch(err) {
      log.err('tunnel changing root or user failed %j %s', msg, err);
      process.exit(8);
    }

    var bindAddress = '127.0.0.1';
    if (forward.length === 4) {
      bindAddress = forward.shift();
    }

    tunnel.on('ready', function() {
      log.notice('tunnel connected to %s', msg.host);
      tunnel.forwardOut(bindAddress, forward[0], forward[1], forward[2], function(err, channel) {
        if (err) {
          log.err(err);
          process.exit(3);
        }

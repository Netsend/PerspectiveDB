...
StreamTree.prototype._read = function() {
  ...
  determineRange(this._tree, this._opts, function(err, r) {
    if (err) { that.emit('error', err); return; }

    that._tree._log.info('StreamTree ikey range %j', r);

    openStream(that._tree, r, that._opts, function(err, it) {
      that._it = it;

      function processItem(item) {
        that._tree._log.debug('StreamTree item push %j', item);
        ...
        processing--;
        that._tree._log.debug('StreamTree data, processing end: %d', processing);
        process.nextTick(function() {
          if (ended && !processing) {
            that._tree._log.debug('StreamTree data push null, processing:', processing);
            that.push(null);
          }
        });
      }
      ...
      it.on('data', function(obj) {
        processing++;
        that._tree._log.debug('StreamTree data, processing start: %d', processing);
        ...
        if (that._id) {
          ...
          var dsKey = that._tree._composeDsKey(val.id, key.i);

          that._tree._db.get(dsKey, function(err, item) {
            if (err) {
              that._tree._log.err('StreamTree %s', err);
              that.emit('error', err);
              return;
            }
      ...
      it.on('end', function() {
        ended = true;
        if (!processing) {
          that._tree._log.debug('StreamTree end push null, processing:', processing);
          that.push(null);
        }
      });
    });

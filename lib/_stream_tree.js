...
StreamTree.prototype.destroy = function() {
  this.tree._log.debug2('st destroy');
  this._it.destroy();
...
StreamTree.prototype._transform = function(obj, enc, cb) {
  ...
  if (this._id) {
    ...
    var cont = this.push(this._opts.bson ? obj.value : BSON.deserialize(obj.value));
    if (!cont) {
      this.tree._log.debug2('st pause and wait for drain');
      this.pause();
    ...
    this.tree._db.get(dsKey, function(err, item) {
      ...
      var cont = that.push(that._opts.bson ? item : BSON.deserialize(item));
      if (!cont) {
        that.tree._log.debug2('st pause and wait for drain');
        that.pause();

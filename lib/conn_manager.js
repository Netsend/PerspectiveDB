  ...
  function register(conn, opts2) {
    ...
    conn.once('close', function(errOccurred) {
      ...
      if (errOccurred && opts2.reconnectOnError) {
        if (remoteAddr) {
          if (!opts.hide) { console.log('%s: register: reconnecting to %s:%s', programName, remoteAddr, remotePort); }
          register(net.createConnection(remotePort, remoteAddr));
        } else {
          if (!opts.hide) { console.log('%s: register: can not reconnect, no remote address: %s', programName, connId); }
        }
      }
    });
  }

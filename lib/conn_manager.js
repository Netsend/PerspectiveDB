...
function connManager(opts) {
  ...
  function register(conn, opts2) {
    ...
    var address, path, remoteAddr, remotePort;
    ...
    conn.once('close', function(errOccurred) {
      ...
      function scheduleConnAttempt(sec) {
        ...
        setTimeout(function() {
          function errHandler(err) {
            if (!opts.hide) { console.error('%s: register: reconnect %s attempt %s error %s', programName, address, nextAttempt - 1, err); }
          ...
          function connectHandler() {
            ...
            if (!opts.hide) { console.error('%s: register: reconnected to %s in %s attempt(s)', programName, address, nextAttempt - 1); }
            register(newConn);
          }

          var newConn;
          if (path) {
            newConn = net.createConnection(path, connectHandler);
            newConn.path = path;
          } else {
            newConn = net.createConnection(remotePort, remoteAddr, connectHandler);
          }
      ...
      if (errOccurred && opts2.reconnectOnError) {
        if (path || remoteAddr) {
          if (!opts.hide) { console.log('%s: register: reconnecting to %s in %s seconds', programName, address, Math.pow(2, nextAttempt)); }
          scheduleConnAttempt(Math.pow(2, nextAttempt));
        } else {
          if (!opts.hide) { console.error('%s: register: can not reconnect, no path or remote address: %s', programName, connId); }

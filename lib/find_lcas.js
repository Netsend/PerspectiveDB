...
// v is the vertice searched for in E, as soon as leftFound or end of E is
// reached two booleans are returned.
// Since the E is topologically sorted, if subject is on the right, it will be found
// before it's on the left.
function _lr(E, v) {
  var l = false, r = false;
 ...
 *
 * Algorithm:
 *   D1 and D2 are topologically sorted DAGs, iterated from leaf to root (input)
 *   either D1 is a subset of D2 or D2 is a subset of D1
 *   E is an ordered set of edges that represents a new DAG based on vertices
 *   seen in D
 *
 *   lca -> []
 *   E -> []
...
function findLCAs(sX, sY, opts, cb) {
  ...
  sX.once('close', function() {
    sXclosed = true;
    close();
  });

  sY.once('close', function() {
    sYclosed = true;
    close();
  });
   ...
   *         remove e from E
   *         if out-degree(left(e)) == 1
   *           removeVertice(E, left(e))
   *       // delete every single path that originates from v
   *       if left(e) == v
   *         remove e from E
   *         if in-degree(right(e)) == 1
   *           removeVertice(E, right(e))
   *
   * remove a vertice from a topologically sorted DAG
  ...
  function processNext(obj, E, indegree, outdegree, peerE, peerIndegree, peerOutdegree, root) {
    ...
    if (lr.r || root) {
      ...
      if (peerLr.l) {
        ...
        _removeNull(peerE);
        _removeNull(E);
        that._log.debug('peerE and E %j %j', peerE, E);

        // check if there is an edge left in peerE or in E
        if (!peerE.length || !E.length) {

...
/**
 * Determine if v is in left of E and/or right of E.
 *
 ...
 *
 * @param {stream.Readable} sX  readable stream that emits vertices from leaf to
 *                              root
 * @param {stream.Readable} sY  readable stream that emits vertices from leaf to
 *                              root
 * @param {Object} [opts]  object containing configurable parameters
 * @param {Function} cb  First parameter will be an Error or null. Second
 *                       parameter will be an array with all lowest common
 *                       ancestor versions. Third parameter will be an object
 *                       with all lca's from sX by version. Fourth parameter
 *                       will be the same lca's from sY. Fifth parameter will be
 *                       the X root. Sixth parameter will be the Y root.
 *
 * opts:
...
function findLCAs(sX, sY, opts, cb) {
  ...
  if (opts.rootX != null && typeof opts.rootX !== 'object') { throw new TypeError('opts.rootX must be an object'); }
  if (opts.rootY != null && typeof opts.rootY !== 'object') { throw new TypeError('opts.rootY must be an object'); }
  if (opts.log != null && typeof opts.log !== 'object') { throw new TypeError('opts.log must be an object'); }

  var fnv = opts.fnv;
  var rootX = opts.rootX;
  var rootY = opts.rootY;
  ...
  function close() {
    ...
    if (sXclosed && sYclosed && !destroyed) {
      ...
      cb(null, lca, lcaX, lcaY, rootX, rootY);
    }
  }
   ...
   * @param {Object} cache  key is a vertice, value an object
   * @return {Number} number of edges removed
   */
  function removeVertice(E, indegree, outdegree, v, cache) {
    var removed = 0;
    ...
    for (var i = 0; i < E.length; i++) {
      ...
      if (r === v) {
        ...
        if (outdegree[l] === 0) {
          ...
          removed += removeVertice(E, indegree, outdegree, l, cache);
        }
      }
      if (l === v) {
        that._log.debug('find_lcas removeVertice remove');
        ...
        if (indegree[r] === 0) {
          ...
          removed += removeVertice(E, indegree, outdegree, r, cache);
        }
      }
    }
   ...
   */
  function processNext(obj, E, indegree, outdegree, cache, lcaObj, peerE, peerIndegree, peerOutdegree, peerCache, peerLcaObj, root) {
    var o = fnv ? fnv(obj) : obj;
    ...
    if (lr.r || root) {
      ...
      if (peerLr.l) {
        ...
        lca.push(v);
        lcaObj[v] = cache[v];
        peerLcaObj[v] = peerCache[v];
        removeVertice(E, indegree, outdegree, v, cache);
        removeVertice(peerE, peerIndegree, peerOutdegree, v, peerCache);
        _removeNull(E);
        _removeNull(peerE);
        ...
        if (!E.length || !peerE.length) {
          that._log.debug('find_lcas processNext E or peerE empty, pause and close, lca: %s', lca);
          sX.pause();
          sXclosed = true;
  ...
  if (rootX) {
    processNext(rootX, xE, indegreeX, outdegreeX, cacheX, lcaX, yE, indegreeY, outdegreeY, cacheY, lcaY, true);
    sX.on('data', function(vertice) {
      processNext(vertice, xE, indegreeX, outdegreeX, cacheX, lcaX, yE, indegreeY, outdegreeY, cacheY, lcaY);
    });
  } else {
    sX.once('data', function(root) {
      rootX = root;
      processNext(root, xE, indegreeX, outdegreeX, cacheX, lcaX, yE, indegreeY, outdegreeY, cacheY, lcaY, true);
      sX.on('data', function(vertice) {
        processNext(vertice, xE, indegreeX, outdegreeX, cacheX, lcaX, yE, indegreeY, outdegreeY, cacheY, lcaY);
      });
    });
  ...
  if (rootY) {
    processNext(rootY, yE, indegreeY, outdegreeY, cacheY, lcaY, xE, indegreeX, outdegreeX, cacheX, lcaX, true);
    sY.on('data', function(vertice) {
      processNext(vertice, yE, indegreeY, outdegreeY, cacheY, lcaY, xE, indegreeX, outdegreeX, cacheX, lcaX);
    });
  } else {
    sY.once('data', function(root) {
      rootY = root;
      processNext(root, yE, indegreeY, outdegreeY, cacheY, lcaY, xE, indegreeX, outdegreeX, cacheX, lcaX, true);
      sY.on('data', function(vertice) {
        processNext(vertice, yE, indegreeY, outdegreeY, cacheY, lcaY, xE, indegreeX, outdegreeX, cacheX, lcaX);
      });
    });

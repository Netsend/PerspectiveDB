...
Master.prototype.stopTerm = function stopTerm(cb) {
  ...
  this._runningAdapters.forEach(function(adapter, i) {
    tasks.push(function(cb2) {
      ...
      adapter.on('close', function() {
        that._log.notice('master stopTerm closed adapter %d', i);
        cb2();
      });
      adapter.kill();
    });
  ...
  this._tunnels.forEach(function(tunnel) {
    tasks.push(function(cb2) {
      that._log.notice('closing tunnel');
      tunnel.on('close', function() {
        that._log.notice('master stopTerm closed tunnel');
        cb2();
      });
...
Master.prototype._forkAdapter = function _forkAdapter(name, cfg, afterInit, cb) {
  ...
  // open adapter server
  var adapter = spawn(process.execPath, [this._adapters[name]], {
    cwd: process.cwd(),
    env: {},
    stdio: stdio
  });

  var index = this._runningAdapters.length; // save index of the new adapter for later usage
  ...
  adapter.on('exit', function(code, signal) {
    ...
    that._runningAdapters.splice(index, 1);
    if (code > 0) {
      that.stopTerm(function(err) {
        if (err) { that._log.err(err); }
      });
  ...
  adapter.on('close', function(code, signal) {
    that._log.info('%s: adapter child close', name, code, signal);
    // close on premature exit
    if (childPhase !== 'listen') {
      that._log.info('%s: adapter pre-mature exit', name);
      process.kill(process.pid);
    }
    childPhase = 'close';
    that._adapter = null;
...
Master.prototype._startTunnel = function _startTunnel(tunnel, cb) {
  ...
  var te = spawn(process.execPath, [__dirname + '/tunnel_exec'], {
    cwd: process.cwd(),
    env: {},
    stdio: stdio
  });
  ...
  // register tunnel
  var index = this._tunnels.length; // save index of the new tunnel for later usage
  ...
  te.once('close', function(code, sig) {
    that._log.notice('master _startTunnel close', name, code, sig);

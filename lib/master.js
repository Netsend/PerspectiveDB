...
Master.prototype.initDbs = function initDbs(cb) {
  ...
  async.eachSeries(this._dbs, function(dbCfg, cb2) {
    ...
    that._startDb(dbCfg, function(err, db) {
      ...
      // check if there is a local source, if so, setup a version channel, a data channel and start the accompanying adapter
      if (dbCfg.source) {
        tasks.push(function(cb3) {
          var parsedUrl = url.parse(dbCfg.source.url);
          var proto = parsedUrl.protocol.slice(0, proto.length - 1); // strip postfixed ":"
          ...
          that._forkAdapter(proto, dbCfg.source, function afterInit(adapter, dataChannel, versionChannel) {
            db.send({ type: 'localDataChannel' }, dataChannel);
            db.send({ type: 'versionChannel' }, versionChannel);
          }, cb3);
        });
      }
 ...
 *                              will be the version channel file descriptor.
 * @param {Function} cb  First parameter will be an Error object or null.
 *
 ...
 * }
 *
 * Furthermore an adapter should expect a data channel on fd6 and a version channel
 * on fd7.
 */
Master.prototype._forkAdapter = function _forkAdapter(name, cfg, afterInit, cb) {
  ...
  that._log.info('master forkAdapter adapter: %j', Master.filterSecrets(adapterCfg));

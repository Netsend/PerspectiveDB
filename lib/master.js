...
Master.prototype.listen = function listen(cb) {
  ...
  function handleMessage(authReq, conn) {
    ...
    if (!authRequest.valid(authReq)) {
      that._log.err('master listen invalid auth request %j', debugReq(authReq));
    ...
    that._verifyAuthRequest(authReq, function(err) {
      if (err) {
        that._log.err('master listen auth request %j, err %j', debugReq(authReq), err);
        connErrorHandler(conn, err);
        return;
      }

      that._log.info('master listen auth request forwarded %j', debugReq(authReq));

      // now send a stripped auth request and connection to the appropriate db
      var strippedAuthReq = { username: authReq.username };
      if (authReq.hasOwnProperty('offset')) {
        strippedAuthReq.offset = authReq.offset;
      }
      that._dbe[authReq.db].send(strippedAuthReq, conn);
    });
  }
  ...
  var preauthCfg = {
    ...
    wss: !!this._opts.wssKey || !!this._opts.wssCert || !!this._opts.wssDhparam,
    wssHost: this._opts.wss && this._opts.wss.host,
    wssPort: this._opts.wss && this._opts.wss.port,
    chroot: this._opts.chroot,
    user: this._opts.user,
    group: this._opts.group
  };

  preauthCfg.log.file = this._log.getFileStream();
  preauthCfg.log.error = this._log.getErrorStream();
  if (!preauthCfg.log.file && !preauthCfg.log.error) {
    preauthCfg.log.console = true;
  }

  that._log.info('master listen preauth: %j', preauthCfg);
  ...
  preauth.stdout.pipe(process.stdout);
  preauth.stderr.pipe(process.stderr);

  // chroot this process before forking preauth
  chroot(this._opts.chroot || '/var/empty', this._opts.user || 'nobody', this._opts.group || 'nobody');
...
Master.prototype._startDb = function _startDb(cfg, cb) {
  ...
  dbe.stdout.pipe(process.stdout);
  dbe.stderr.pipe(process.stderr);
  ...
  dbe.once('message', function(msg) {
    ...
    dbe.once('message', function(msg) {
      ...
      // setup oplog connection to this dbe
      //that._ensureSnapshotAndOplogOffset(cfg, function(err, offset) {
        //if (err) { cb(err); return; }
...
Master.prototype._verifyAuthRequest = function _verifyAuthRequest(req, cb) {
  ...
  cfg.perspectives.some(function(persCfg) {
    if (persCfg.name === req.username) {
      userDb = persCfg.users;
      replCfg = persCfg;
      return true;
    }
  });

  if (!userDb || (!replCfg.export && !replCfg.import)) {
    error = 'invalid credentials';
  ...
  User.find(userDb, req.username, req.db, function(err, user) {
    if (err) { cb(err); return; }
    if (!user) {
      error = 'invalid credentials';
    ...
    that._log.info('master _verifyAuthRequest %s user found %j', req.db, user);
    ...
    user.verifyPassword(req.password, function(err, valid) {
      ...
      if (!valid) {
        error = 'invalid credentials';
        that._log.err('master _verifyAuthRequest', req.db, error);
        cb(new Error(error));
        return;
      }

      that._log.info('master successfully authenticated', req.username);

 ...
 *   dbroot:       {String}        database root directory, default is /var/pdb
 ...
 *   port:         {Number}        tcp port for local tcp server, defaults to 2344
...
function Master(dbs, opts) {
  ...
  if (opts.port != null && typeof opts.port !== 'number') { throw new TypeError('opts.port must be a number'); }
    ...
    async.map(files.map(file => apath + file), fs.stat, function(err, result) {
      ...
      result.forEach(function(stat, i) {
        if (stat.isDirectory()) {
          ...
          if (stats.isFile()) {
            that._log.debug('master adapter %s loaded', name);
...
Master.prototype.initClients = function initClients(cb) {
  ...
  async.eachSeries(this._dbs, function(dbCfg, cb2) {
    async.eachSeries(dbCfg.perspectives, function(peCfg, cb3) {
      ...
      that._log.debug2('master initClients %s loading %s', dbName, file);
      if (file && file[0] !== '/') {
      ...
      that._startClient(peCfg, function(err, ceChild) {
        ...
        ceChild.on('message', function handleMessage(msg, conn) {
          ...
          // data that arrives before the child has the socket will be lost. since this function operates synchronously this should not be a problem here.
          // https://github.com/nodejs/node/pull/6951
          conn.on('readable', function() {
            that._log.err('master initClients data or state received that will be lost');
          });

          // forward the connection to the appropriate db
          msg = {
            type: 'remoteDataChannel',
            perspective: peCfg.name,
            receiveBeforeSend: true
          };
          that._dbe[dbCfg.name].send(msg, conn);
        });
        cb3();
      });
    }, cb2);
...
Master.prototype.initDbs = function initDbs(cb) {
  ...
  async.eachSeries(this._dbs, function(dbCfgSecret, cb2) {
    ...
    var localDataChannelSent = false;
    that._startDb(dbCfg, function(err, db) {
      ...
      if (dbCfg.source) {
        if (localDataChannelSent) {
          error = new Error('second source definition encountered');
      ...
      tasks.push(function(cb3) {
        // signal that it should start auto merging if no local data channel is setup
        if (!localDataChannelSent) {
          db.send({ type: 'autoMerge' });
        }
...
Master.prototype.listen = function listen(cb) {
  ...
  function connErrorHandler(conn, e, debugData) {
    try {
      var msg = { error: 'invalid auth request' };
      that._log.debug('master listen %s %j', e, debugData);
  ...
  function handleMessage(authReq, conn) {
    that._log.debug2('master listen %d %d %j', conn.bytesRead, conn.bytesWritten, conn.address());

    // data that is received before the child reveived the socket will be lost
    // https://github.com/nodejs/node/pull/6951
    conn.on('readable', function() {
      that._log.warning('master listen data or state received that will be lost');
    });

    if (!authRequest.valid(authReq)) {
      connErrorHandler(conn, new Error('invalid auth request'), filterSecrets(authReq));
      return;
    }
    ...
    that._verifyAuthRequest(authReq, function(err) {
      if (err) {
        connErrorHandler(conn, err, filterSecrets(authReq));
        return;
      }
      ...
      var msg = {
        type: 'remoteDataChannel',
        perspective: authReq.username,
        receiveBeforeSend: false
      };
      that._dbe[authReq.db].send(msg, conn);
    });
  }

  that._log.debug2('master listen forking preauth');
  ...
  that._log.debug2('master listen preauth: %j', filterSecrets(preauthCfg));
  ...
  preauth.once('message', function(msg) {
    ...
    that._log.debug2('master listen "init" received');
    ...
    preauth.once('message', function(msg) {
      ...
      that._log.debug2('master listen "listen" received');
...
Master.prototype.stopTerm = function stopTerm(cb) {
  ...
  this._log.info('master stopTerm clients %d', this._clients.length);

  this._clients.forEach(function(client, i) {
    if (client && client.connected) {
      that._log.debug2('closing client %d', i);
      tasks.push(function(cb2) {
        client.on('exit', function() {
          that._log.debug('closed client %d', i);
          cb2();
        });
        that._log.debug('sending kill signal to client %d', i);
        client.send({ type: 'kill' });
      });
    } else {
      that._log.info('client %d already closed', i);
    }
  });

  var dbe = Object.keys(this._dbe);
  this._log.info('master stopTerm dbe', dbe);

  dbe.forEach(function(key) {
    var dbe = that._dbe[key];
    if (dbe && dbe.connected) {
      tasks.push(function(cb2) {
        that._log.debug('closing dbe', key);
        dbe.on('exit', function() {
          that._log.debug('master stopTerm closed dbe', key);
          cb2();
        });
        dbe.send({ type: 'kill' });
      });
    } else {
      that._log.info('dbe already closed', key);
    }
  });
    ...
    tasks.push(function(cb2) {
      if (that._preauth && that._preauth.connected) {
        that._preauth.on('exit', function(err) {
          if (err) { cb2(err); return; }
          that._log.debug2('preauth closed');
          cb2();
        });
        that._log.debug('closing preauth');
        that._preauth.send({ type: 'kill' });
      } else {
        that._log.info('preauth already closed');
      }
    });
  }

  this._log.info('master stopTerm adapters %d', this._runningAdapters.length);

  this._runningAdapters.forEach(function(adapter, i) {
    if (adapter && adapter.connected) {
      that._log.debug2('closing adapter %d', i);
      tasks.push(function(cb2) {
        adapter.on('exit', function() {
          that._log.debug('closed adapter %d', i);
          cb2();
        });
        that._log.debug('sending kill signal to adapter %d', i);
        adapter.send({ type: 'kill' });
      });
    } else {
      that._log.info('adapter %d already closed', i);
    }
  });

  this._log.info('master stopTerm tunnels %d', this._tunnels.length);

  this._tunnels.forEach(function(tunnel, i) {
    if (tunnel && tunnel.connected) {
      that._log.debug2('closing tunnel %d', i);
      tasks.push(function(cb2) {
        tunnel.on('exit', function() {
          that._log.debug('closed tunnel %d', i);
          cb2();
        });
        that._log.debug('sending kill signal to tunnel %d', i);
        tunnel.send({ type: 'kill' });
      });
    } else {
      that._log.info('tunnel %d already closed', i);
    }
  });
...
Master.prototype._forkAdapter = function _forkAdapter(name, cfg, afterInit, cb) {
  ...
  this._log.debug2('master forkAdapter %s', name);
  ...
  that._log.debug2('master forkAdapter adapter: %j', filterSecrets(adapterCfg));
  ...
  adapter.once('message', function(msg) {
    ...
    that._log.debug2('master forkAdapter "init" received');
    ...
    adapter.once('message', function(msg) {
      ...
      childPhase = 'listen';
      that._log.debug2('master forkAdapter "listen" received');
      cb();
    });
  });
};
...
Master.prototype._startTunnel = function _startTunnel(tunnel, cb) {
  ...
  that._log.debug2('master _startTunnel forking: %s', name);
  ...
  // prepend the config base to a relative path specification
  let file = tunnel.key;
  this._log.debug2('master _startTunnel %s loading %s', tunnel.connect, file);
  if (file && file[0] !== '/') {
  ...
  te.once('message', function(msg) {
    ...
    phase = 'init';
    that._log.debug2('master _startTunnel "init" received', name);
    ...
    te.once('message', function(msg) {
      ...
      that._log.debug2('master _startTunnel "listen" received', name);
 ...
 *   [log]:          {Object}      // log configuration
 *   [port]:         {Number}      // port, might be a tunnel. defaults to master port
...
Master.prototype._startClient = function _startClient(cfg, cb) {
  ...
  cfg.port = cfg.port || this._opts.port;
  var name = cfg.name || cfg.port;
  var that = this;

  that._log.debug2('master _startClient forking: %s', name);
  ...
  ce.once('message', function(msg) {
    ...
    phase = 'init';
    that._log.debug2('master _startClient "init" received', name);
    ...
    ce.once('message', function(msg) {
      ...
      that._log.debug2('master _startClient "listen" received', name);
 ...
 *   [log]:          {Object}      // log configuration
 *   [chroot]:       {String}      // defaults to dbroot + name
...
Master.prototype._startDb = function _startDb(cfg, cb) {
  ...
  if (!cfg.name || typeof cfg.name !== 'string') { throw new TypeError('cfg.name must be a non-empty string'); }
  ...
  cfg = xtend({
    chroot: this._opts.dbroot + '/' + cfg.name,
  }, cfg);
  cfg.hookPaths = cfg.hookPaths || [__dirname + '/../hooks/core', __dirname + '/../hooks/local'];

  var name = cfg.name;
  var that = this;

  that._log.debug2('master _startDb forking: %s', name);
  ...
  dbe.once('message', function(msg) {
    ...
    phase = 'init';
    that._log.debug2('master _startDb "init" received', name);
    ...
    dbe.once('message', function(msg) {
      ...
      that._log.debug2('master _startDb "listen" received', name);

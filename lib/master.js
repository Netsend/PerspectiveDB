  ...
  async.eachSeries(this._dbs, function(dbCfg, cb2) {
    async.eachSeries(dbCfg.perspectives, function(peCfg, cb3) {
      ...
      if (peCfg.secrets) {
        ...
        that._log.debug('master initClients %s loading %s', peCfg.name, file);
        if (file && file[0] !== '/') {
          file = that._opts.configBase + file;
        }
        try {
          let secrets = loadSecrets(file);
          peCfg.password = secrets[peCfg.username];
          that._log.debug('master initClients %s loaded %s', peCfg.name, file);
        } catch(err) {
          that._log.err('master initClients %s loading %s %s', peCfg.name, file, err);
    ...
    that._startDb(dbCfg, function(err, db) {
      ...
      if (dbCfg.source) {
        tasks.push(function(cb3) {
          ...
          if (dbCfgSecret.source.passdb) {
            // prepend the config base to a relative path specification
            let file = dbCfgSecret.source.passdb;
            that._log.debug('master initDbs %s loading %s', dbCfg.source.url, file);
            if (file && file[0] !== '/') {
              file = that._opts.configBase + file;
            }
            try {
              dbCfg.source.passdb = loadSecrets(file);
 ...
 * remote hosts.
 *
 * @param {Object} client          client exec config
 * @param {Function} cb            First parameter will be an error object or
 *                                 null, second parameter will be the forked child
 *                                 on success.
 *
 ...
 * }
 */
Master.prototype._startClient = function _startClient(client, cb) {
  if (typeof client !== 'object') { throw new TypeError('client must be an object'); }
  if (typeof cb !== 'function') { throw new TypeError('cb must be a function'); }

  if (client.log == null) { client.log = this._opts.log; }
  if (typeof client.log !== 'object') { throw new TypeError('client.log must be an object'); }

  if (!client.username || typeof client.username !== 'string') { throw new TypeError('client.username must be a string'); }
  if (!client.password || typeof client.password !== 'string') { throw new TypeError('client.password must be a string'); }
  if (!client.database || typeof client.database !== 'string') { throw new TypeError('client.database must be a string'); }

  if (client.port != null && typeof client.port !== 'number') { throw new TypeError('client.port must be an array'); }
  if (client.name != null && !Array.isArray(client.name)) { throw new TypeError('client.name must be an array'); }
  if (client.chroot != null && typeof client.chroot !== 'string') { throw new TypeError('client.chroot must be a string'); }
  if (client.user != null && typeof client.user !== 'string') { throw new TypeError('client.user must be a string'); }
  if (client.group != null && typeof client.group !== 'string') { throw new TypeError('client.group must be a string'); }

  var name = client.name || client.connect;
  var that = this;
  ...
  // use fd 4 if a file stream is opened
  if (client.log.file) {
    stdio[4] = client.log.file.fd;
    client.log.file = 4;
  }

  // use fd 5 if an error stream is opened
  if (client.log.error) {
    stdio[5] = client.log.error.fd;
    client.log.error = 5;
  }
  ...
  this._log.notice('client registered %s', client.connect);

 ...
 *   [name]:         {String}      // name of this database, defaults to "pdb"
 ...
 *   prefixFirst: {String}  prefix to search for, mutually exclusive with id,
...
function headLookupConnHandler(conn, mt) {
  ...
  conn.pipe(ls).on('readable', function() {
    ...
    } else if (req.prefixFirst) {
      var head;
      ltree.getHeads({ prefix: req.prefixFirst, bson: true, limit: 1 }, function(h, next) {
        head = h;
        next();
      }, function(err) {
        if (err) { connErrorHandler(conn, connId, err); return; }
...
process.once('message', function(msg) {
  ...
  var newRoot = msg.chroot || '/var/pdb';

...
function headLookupConnHandler(conn, mt) {
  log.debug2('head lookup channel %s %j', createConnId(conn), conn.address());
...
function localDataConnHandler(conn, mt) {
  log.debug2('local data channel %s %j', createConnId(conn), conn.address());
...
function remoteDataConnHandler(conn, mt, pers, receiveBeforeSend) {
  ...
  function finalize() {
    ...
    if (!dataReqSent && receiveBeforeSend && dataReqReceived && dataReq) {
      log.debug2('setup pipes and send back data request %j', dataReq);
  ...
  if (pers.import) {
    ...
    log.debug('signal that no data is expected');

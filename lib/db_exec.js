 ...
 *
 * Any subsequent messages sent to this process should be data channel requests.
 * These should be accompanied with a file descriptor and a perspective.
 *
 * {
 ...
 * }
 *
 * On data channels a data request is sent back to the client. After this a data request
 * is expected from the other side so that the real data exchange (BSON) can be started.
 *
...
function postChroot(cfg) {
  ...
  // start auto-merging
  mt.mergeAll({ interval: mtOpts.autoMergeInterval || 5000 });

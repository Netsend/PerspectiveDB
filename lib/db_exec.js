...
function postChroot(db, cfg) {
  ...
  function handleIncomingMsg(req, conn) {
    ...
    if (conn) {
      ...
      if (!pers) {
        log.err('mtr handleIncomingMsg "unknown remote" %j', req);
      ...
      if (pers.import) {
        ...
        mt.lastByPerspective(remote, 'base64', function(err, last) {
          if (err) {
            log.err('mtr handleIncomingMsg lastByPerspective %s %j', err, req);
          ...
          rtOpts.hooksOpts.to = db;

          var rt = new RemoteTransform(remote, rtOpts);

          rt.on('error', function(err) {
            log.err('remote transform error %s %j', err, conn.address());
          });

          // expect bson
          var bs = new BSONStream();
          var mtw = mt.createRemoteWriteStream();

          conn.pipe(bs).pipe(rt).pipe(mtw);

          conn.once('end', function() {
            conn.unpipe(bs);
            log.notice('conn end unpiped conn--bs');
            bs.end();
          });

          bs.once('end', function() {
            bs.unpipe(rt);
            log.notice('bs end unpiped bs--rt ');
            rt.end();
          });

          rt.once('end', function() {
            rt.unpipe(mtw);
            log.notice('rt end unpiped rt--mtw');
          });
          ...
          log.notice('sending auth response %j', authRes);
          conn.write(JSON.stringify(authRes) + '\n');

          if (!authRes.start) {
            log.notice('close our half of the connection since we don\'t want data');
            conn.end();
          }

          // pass requested data to remote
          mtr.pipe(conn);
        });
      } else {
        // signal that not data is expected
  ...
  logger(msg.log, function(err, l) {
    ...
    fs.stat(newRoot + path, function(err, stats) {
      if (err && err.code !== 'ENOENT') {
        log.err('stats on path failed %s %j', err, msg);
        process.exit(4);
      }
      ...
      if (err && err.code === 'ENOENT') {
        fs.mkdir(newRoot + path, 0o755, function(err) {
          if (err) {
            log.err('path creation failed %s %j', err, msg);
            process.exit(5);
          }

          fs.chown(newRoot + path, uid, gid, function(err) {
            if (err) {
              log.err('setting path ownership failed %s %j', err, msg);
              process.exit(6);
            }
        ...
        if (!stats.isDirectory()) {
          log.err('path exists but is not a directory %j %j', stats, msg);
          process.exit(7);
        }
        doChroot();
        openDbAndProceed();

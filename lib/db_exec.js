 ...
 * requests and a startMerge signal.
 *
 * Any subsequent messages sent to this process should be data channel requests or
 * a startMerge signal. Data channel requests should be accompanied with a file
 * descriptor and a perspective.
 *
 * {
 ...
 * }
 *
 * A local data channel is used to update the local tree. Only one local data channel
 * can be active. Received versions must be either new versions or confirmations of
 * merged versions. If a new version is created by a remote the merge is sent back
 * with the previous head on the connection.
 *
 * {
 ...
 * - maybe import data
 */
function remoteDataConnHandler(conn, mt, pers) {
  log.info('client connected %s %j', createConnId(conn), conn.address());
  ...
  function handleIncomingMsg(msg, conn) {
    ...
    switch (msg.type) {
      ...
      case 'remoteDataChannel':
        ...
        remoteDataConnHandler(conn, mt, pers);
        break;
      default:
  ...
  logger(msg.log, function(err, l) {
    ...
    function doChroot() {
      try {
        ...
        log.notice('changed root to %s and user:group to %s:%s', newRoot, user, group);
      } catch(err) {

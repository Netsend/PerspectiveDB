...
function postChroot(db, cfg) {
  ...
  log.info('dbe persCfg %j', persCfg);
  ...
  function handleIncomingMsg(req, conn) {
    ...
    registerIncoming(conn, remote, function(err) {
      ...
      mtr.once('close', function() {
        log.notice('dbe mtr end unpiped mtr--conn');
        mtr.unpipe(conn);
      });
      ...
      if (pers.import) {
        ...
        mt.lastByPerspective(remote, 'base64', function(err, last) {
          if (err) {
            log.err('dbe mtr handleIncomingMsg lastByPerspective %s %j', err, req);
          ...
          rt.on('error', function(err) {
            log.err('dbe remote transform error %s %j', err, conn.address());
          });

          // expect bson
          var bs = new BSONStream();
          var mtw = mt.createRemoteWriteStream(remote);

          conn.pipe(bs).pipe(rt).pipe(mtw);

          conn.once('end', function() {
            log.notice('dbe conn end unpiped conn--bs');
            conn.unpipe(bs);
            bs.end();
          });

          bs.once('end', function() {
            log.notice('dbe bs end unpiped bs--rt ');
            bs.unpipe(rt);
            rt.end();
          });

          rt.once('end', function() {
            log.notice('dbe rt end unpiped rt--mtw');
            rt.unpipe(mtw);
            mtw.end();
          });
          ...
          log.notice('dbe sending auth response %j', authRes);
        ...
        log.notice('dbe signal that no data is expected and close this half of the stream');
        conn.end(JSON.stringify({ start: false }) + '\n');
...
process.once('message', function(msg) {
  ...
  logger(msg.log, function(err, l) {
    ...
    try {
      ...
      log.err('dbe %s %s ', err, user, group);
      process.exit(3);
    }

...
/**
 * Instantiates new merge trees and handle incoming oplog items on stdin.
 *
 ...
 * receive the configuration data. This is the database config, a db config and the
 * log configuration. File desciptors for the log config should be passed while
 * forking this process and metioned in the log config.
 *
 * {
 *   dbName:          {String}
...
function registerVcr(vcr, conn) {
  ...
  log.notice('vcr registering %s', dest);

  vcrs.push(vcr);

  vcr.on('error', function(err) {
    log.err('vcr error %s %s', dest, err);
  });

  vcr.once('end', function() {
    log.notice('vcr end %s', dest);
...
function startDb(db, cfg) {
  ...
  var mt = new MergeTree(db, cfg.collectionName, cfg);
  mt.startAutoProcessing(cfg.autoProcessInterval);
  ...
  process.stdin.pipe(bs).on('data', function(obj) {
    mt.saveOplogItem2(obj, function(err) {
      if (err) {
    ...
    if (mt._queueLimit <= mt._oplogBuffer.length) {
      log.warning('saveOplogItem queue full, wait and retry.', mt._queueLimitRetryTimeout, mt._oplogBuffer.length);

      bs.pause();

      setTimeout(function() {
        log.info('saveOplogItem retry', mt._queueLimitRetryTimeout, mt._oplogBuffer.length);

        bs.resume();
      }, mt._queueLimitRetryTimeout);
    }
  });
  ...
  function handlePullReq(pullReq) {
    ...
    mt.lastByPerspective(database, function(err, last) {
...
// expect an init request (request that contains db path, db name, a log config and any other options)
process.once('message', function(msg) {
  if (typeof msg !== 'object') { throw new TypeError('msg must be an object'); }
  if (typeof msg.dbPath !== 'string') { throw new TypeError('msg.dbPath must be a string'); }
  ...
  programName = 'dbe ' + msg.dbName;

  process.title = 'pd/' + programName;
  ...
  logger(msg.logCfg, function(err, l) {
    ...
    log = l;
    msg.log = l; // use this logger in the mt's as well
    ...
    // open database
    level(msg.dbPath, { keyEncoding: 'binary', valueEncoding: 'binary' }, function(err, db) {
      if (err) {

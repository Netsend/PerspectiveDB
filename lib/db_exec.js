...
function connHandler(conn, mt, pers) {
  ...
  if (pers.import) {
    ...
    mt.lastReceivedFromRemote(pers.name, 'base64', function(err, last) {
      if (err) {
        log.err('connHandler lastReceivedFromRemote %s %s', err, pers.name);
        connErrorHandler(conn, connId, err);
        return;
      }
...
function postChroot(cfg) {
  ...
  function handleIncomingMsg(req, conn) {
    if (!conn) {
      log.err('handleIncomingMsg connection missing %j', req);
      return;
    }

    log.notice('incoming ipc message %j', req);
